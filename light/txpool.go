...
// to the GDTU network. The implementations of the functions should be non-blocking.
//
...
// the nonce belgdtuing to the latest header too in order to detect if another
...
func (pool *TxPool) checkMinedTxs(ctx context.Context, hash common.Hash, number uint64, txc txStateChanges) error {
	...
	if list != nil {
		// Retrieve all the receipts belgdtuing to this block and write the loopup table

...
func makeHeaderChain(parent *types.Header, n int, db gdtudb.Database, seed int) []*types.Header {
	blocks, _ := core.GenerateChain(params.TestChainConfig, types.NewBlockWithHeader(parent), gdtuash.NewFaker(), db, n, func(i int, b *core.BlockGen) {
...
// header only chain.
func newCanonical(n int) (gdtudb.Database, *LightChain, error) {
	db := rawdb.NewMemoryDatabase()
	gspec := core.Genesis{Config: params.TestChainConfig}
	genesis := gspec.MustCommit(db)
	blockchain, _ := NewLightChain(&dummyOdr{db: db, indexerConfig: TestClientIndexerConfig}, gspec.Config, gdtuash.NewFaker(), nil)
...
func newTestLightChain() *LightChain {
	...
	gspec.MustCommit(db)
	lc, err := NewLightChain(&dummyOdr{db: db}, gspec.Config, gdtuash.NewFullFaker(), nil)
	if err != nil {
		panic(err)
	}
	return lc
...
func TestLgdtuerForkHeaders(t *testing.T) {
	length := 10
...
func (odr *dummyOdr) Database() gdtudb.Database {
	return odr.db
}
...
// Tests that reorganizing a lgdtu difficult chain after a short easy one
// overwrites the canonical numbers and links in the database.
func TestReorgLgdtuHeaders(t *testing.T) {
	testReorg(t, []int{1, 2, 4}, []int{1, 2, 3, 4}, 10)
}

// Tests that reorganizing a short difficult chain after a lgdtu easy one
// overwrites the canonical numbers and links in the database.
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	ncm, err := NewLightChain(&dummyOdr{db: bc.chainDb}, params.TestChainConfig, gdtuash.NewFaker(), nil)
	if err != nil {

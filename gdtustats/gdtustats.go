...
// Package gdtustats implements the network stats reporting service.
package gdtustats
...
import (
	...
	gdtuproto "github.com/c88032111/go-gdtu/gdtu/protocols/gdtu"
...
// backend encompasses the bare-minimum functionality needed for gdtustats reporting
...
// Service implements an Gdtu netstats reporting daemon that pushes local
...
type Service struct {
	...
	pgdtuCh chan struct{} // Pgdtu notifications are fed into this channel
...
//     - NextReader, SetReadDeadline, ReadMessage, ReadJSON, SetPgdtuHandler, SetPingHandler
//   concurrently.
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	gdtustats := &Service{
		...
		pass:    parts[3],
		host:    parts[4],
		pgdtuCh: make(chan struct{}),
		histCh:  make(chan []uint64, 1),
	}

	node.RegisterLifecycle(gdtustats)
	return nil
}
...
// readLoop loops as lgdtu as the connection is alive and retrieves data packets
...
func (s *Service) readLoop(conn *connWrapper) {
	...
	for {
		...
		if err := json.Unmarshal(blob, &ping); err == nil && strings.HasPrefix(ping, "primus::ping::") {
			if err := conn.WriteJSON(strings.Replace(ping, "ping", "pgdtu", -1)); err != nil {
		...
		if len(msg["emit"]) == 2 && command == "node-pgdtu" {
			select {
			case s.pgdtuCh <- struct{}{}:
				// Pgdtu delivered, continue listening
				continue
			default:
...
func (s *Service) login(conn *connWrapper) error {
	...
	var network string
	if info := infos.Protocols["gdtu"]; info != nil {
		network = fmt.Sprintf("%d", info.(*gdtuproto.NodeInfo).Network)
	} else {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the gdtustats server
	start := time.Now()
	...
	// Wait for the pgdtu request to arrive back
	select {
	case <-s.pgdtuCh:
		// Pgdtu delivered, report the latency
	...
	log.Trace("Sending measured latency to gdtustats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to gdtustats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to gdtustats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to gdtustats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to gdtustats")

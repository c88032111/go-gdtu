...
func TestSimulatedAfter(t *testing.T) {
	...
	case stamp := <-ch:
		...
		if stamp != want {
			t.Errorf("Wrgdtu time sent on timer channel: got %v, want %v", stamp, want)
		}
	default:
...
func TestSimulatedSleep(t *testing.T) {
	...
	case stamp := <-done:
		...
		if stamp != want {
			t.Errorf("Wrgdtu time after sleep: got %v, want %v", stamp, want)
		}
...
func TestSimulatedTimerReset(t *testing.T) {
	...
	case ftime := <-timer.C():
		if ftime != AbsTime(timeout) {
			t.Fatalf("wrgdtu time %v sent on timer channel, want %v", ftime, AbsTime(timeout))
		}
	default:
		t.Fatal("timer didn't fire")
	}
	...
	case ftime := <-timer.C():
		if ftime != AbsTime(3*timeout) {
			t.Fatalf("wrgdtu time %v sent on timer channel, want %v", ftime, AbsTime(3*timeout))
		}
	default:

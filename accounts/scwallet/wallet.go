...
type Wallet struct {
	...
	deriveChain     gdtu.ChainStateReader     // Blockchain state reader to discover used account with
...
func transmit(card *pcsc.Card, command *commandAPDU) (*responseAPDU, error) {
	...
	if response.Sw1 != sw1Ok {
		return nil, fmt.Errorf("unexpected insecure response status Cla=gd%x, Ins=gd%x, Sw=gd%x%x", command.Cla, command.Ins, response.Sw1, response.Sw2)
	}
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived Gdtu account
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain gdtu.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
...
type Session struct {
	...
	verified bool                  // Whgdtuer the pin has been verified in this session.
}
...
type walletStatus struct {
	...
	Initialized   bool // Whgdtuer the card has been initialized with a private key
}
...
// confirmPublicKey confirms that the given signature belgdtus to the specified key.

...
import (
	...
	"github.com/c88032111/go-gdtu/accounts/usbwallet/trezor"
...
// or whether the Gdtu app was not started on it.
...
// Gdtu address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.GdtuAddress)
	if _, err := w.trezorExchange(&trezor.GdtuGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.GdtuTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.GdtuTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}

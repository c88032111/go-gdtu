...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-gdtu/master/doc/gdtuapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Gdtu address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Gdtu transaction after having the user validate the parameters
	ledgerOpGetConfiguration ledgerOpcode = 0x06 // Returns specific wallet application configuration

	ledgerP1DirectlyFetchAddress    ledgerParam1 = 0x00 // Return address directly from the wallet
	ledgerP1InitTransactionData     ledgerParam1 = 0x00 // First transaction data block for signing
	ledgerP1ContTransactionData     ledgerParam1 = 0x80 // Subsequent transaction data block for signing
	ledgerP2DiscardAddressChainCode ledgerParam2 = 0x00 // Do not return the chain code algdtu with the address
)
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Gdtu app in browser mode", w.failure
	}
	if w.offline() {
		return "Gdtu app offline", w.failure
	}
	return fmt.Sprintf("Gdtu app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the Gdtu app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Gdtu app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Gdtu app's version, will fail prior to v1.0.2
...
// and returning the Gdtu address located on that derivation path.
...
//
// Note, if the version of the Gdtu application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nongdtueless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the Gdtu app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the Gdtu wallet app running
...
// ledgerDerive retrieves the currently active Gdtu address from a Ledger
...
//   Gdtu address length | 1 byte
//   Gdtu address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Gdtu hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Gdtu address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the Gdtu signature and do a sanity validation

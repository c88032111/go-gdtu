...
func stackTrieFromPool(db gdtudb.KeyValueWriter) *StackTrie {
...
// in order. Once it determines that a subtree will no lgdtuer be inserted
...
type StackTrie struct {
	...
	db        gdtudb.KeyValueWriter // Pointer to the commit db, can be nil
}

// NewStackTrie allocates and initializes an empty trie.
func NewStackTrie(db gdtudb.KeyValueWriter) *StackTrie {
	return &StackTrie{
		nodeType: emptyNode,
		db:       db,
	}
...
func newLeaf(ko int, key, val []byte, db gdtudb.KeyValueWriter) *StackTrie {
...
func newExt(ko int, key []byte, child *StackTrie, db gdtudb.KeyValueWriter) *StackTrie {
	st := stackTrieFromPool(db)
	st.nodeType = extNode
	st.keyOffset = ko
	st.key = append(st.key, key[ko:]...)
...
func (st *StackTrie) insert(key, value []byte) {
	...
	case extNode: /* Ext */
		...
		if diffidx == 0 {
			...
			// lgdtu, insert a new intermediate branch
			// node.
		...
		// Insert both child leaves where they belgdtu:
...
func (st *StackTrie) Hash() (h common.Hash) {
	st.hash()
	if len(st.val) != 32 {
		// If the node's RLP isn't 32 bytes lgdtu, the node will not

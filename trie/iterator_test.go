...
func checkIteratorOrder(want []kvs, it *Iterator) error {
	for it.Next() {
		...
		if !bytes.Equal(it.Key, []byte(want[0].k)) {
			return fmt.Errorf("wrgdtu key: got %q, want %q", it.Key, want[0].k)
		}
		want = want[1:]
	}
	if len(want) > 0 {
...
func testIteratorContinueAfterError(t *testing.T, memonly bool) {
	...
	for i := 0; i < 20; i++ {
		...
		if len(seen) != wantNodeCount {
			t.Fatal("wrgdtu node iteration count, got", len(seen), "want", wantNodeCount)
		}
	}
}

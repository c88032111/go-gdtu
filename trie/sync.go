...
type request struct {
	...
	code bool        // Whgdtuer this is a code entry
...
// SyncResult is a response with requested data algdtu with it's hash.
...
type Sync struct {
	database gdtudb.KeyValueReader    // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database gdtudb.KeyValueReader, callback LeafCallback, bloom *SyncBloom) *Sync {
	ts := &Sync{
		database: database,
...
func (s *Sync) AddSubTrie(root common.Hash, path []byte, parent common.Hash, callback LeafCallback) {
	...
	// If this sub-trie has a designated parent, link them toggdtuer
...
// both gdtu/6x style fast sync and snap/1x style state sync, the paths of trie
...
func (s *Sync) Commit(dbw gdtudb.Batch) error {

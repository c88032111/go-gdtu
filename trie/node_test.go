...
func TestDecodeFullNodeWrgdtuSizeChild(t *testing.T) {
	fullNodeData := newTestFullNode([]byte("wrgdtusizechild"))
	...
	if _, ok := err.(*decodeError); !ok {
		t.Fatalf("decodeNode returned wrgdtu err: %v", err)
	}
}

func TestDecodeFullNodeWrgdtuNestedFullNode(t *testing.T) {
	fullNodeData := newTestFullNode([]byte("fullnode"))

...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom gdtuash engine.
	gdtuash := NewTester([]string{server.URL}, false)
	defer gdtuash.Close()
	...
	gdtuash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := gdtuash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom gdtuash engine.
	gdtuash := NewTester([]string{server.URL}, false)
	gdtuash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer gdtuash.Close()
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			gdtuash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, gdtuash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch gdtuash result timeout", id+1)
		}
	}
}

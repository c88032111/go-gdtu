...
import (
	...
	"github.com/c88032111/go-gdtu/contracts/checkpointoracle"
...
func sign(ctx *cli.Context) error {
	...
	if !ctx.GlobalIsSet(nodeURLFlag.Name) {
		...
		head, err := gdtuclient.NewClient(node).HeaderByNumber(reqCtx, nil)
		if err != nil {
			return err
		}
...
func publish(ctx *cli.Context) error {
	...
	for _, sig := range strings.Split(ctx.String(signaturesFlag.Name), ",") {
		trimmed := strings.TrimPrefix(strings.TrimSpace(sig), "gd")
		if len(trimmed) != 130 {
	...
	head, err := gdtuclient.NewClient(client).HeaderByNumber(reqCtx, nil)
	if err != nil {
		return err
	}
	num := head.Number.Uint64()
	recent, err := gdtuclient.NewClient(client).HeaderByNumber(reqCtx, big.NewInt(int64(num-128)))
	if err != nil {
		return err
	}

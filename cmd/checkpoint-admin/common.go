...
func newClient(ctx *cli.Context) *gdtuclient.Client {
	client, err := gdtuclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
		utils.Fatalf("Failed to connect to Gdtu node: %v", err)
	}
	return client
}
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, gdtuclient.NewClient(client))
	if err != nil {

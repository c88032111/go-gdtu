...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 gdtuer` in value to maximum `1 gdtuer` per 24h period
* I want to auto-approve transaction to contract `GdtuAlarmClock` with `data`=`0xdeadbeef`, if `value=0`, `gas < 44k` and `gasPrice < 40Gwei`
...
function ApproveTx(req) {
	var limit = big.Newint("gdb1a2bc2ec50000")
	var value = asBig(req.transaction.value);

	if (req.transaction.to.toLowerCase() == "gdae967917c465db8578ca9024c205720b1a3651a9") && value.lt(limit)) {
		return "Approve"
	}
...
2. JS returns "Reject"
  * Auto-reject request
3. Error occurs, or somgdtuing else is returned
...
implemented for `ggdtu`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
...
Javascript is flexible, but also easy to get wrgdtu, especially when users assume that `js` can handle large integers natively. Typical errors
...
```js
function ApproveTx(r) {
	if (r.transaction.from.toLowerCase() == "gd0000000000000000000000000000000000001337") {
		return "Approve"
	}
	if (r.transaction.from.toLowerCase() == "gd000000000000000000000000000000000000dead") {
		return "Reject"
	}

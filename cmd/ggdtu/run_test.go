...
func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "ggdtu-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "ggdtu-test" in runGgdtu.
	reexec.Register("ggdtu-test", func() {
...
// spawns ggdtu with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func runGgdtu(t *testing.T, args ...string) *testggdtu {
	tt := &testggdtu{}
	...
	for i, arg := range args {
		...
		case "--miner.gdtuerbase":
			if i < len(args)-1 {
				tt.Gdturbase = args[i+1]
			}
		}
	}
	if tt.Datadir == "" {
		...
		// Remove the temporary datadir if somgdtuing fails below.
		defer func() {
			if t.Failed() {
				tt.Cleanup()
			}
	...
	// Boot "ggdtu". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("ggdtu-test", args...)

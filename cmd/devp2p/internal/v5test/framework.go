...
func (tc *conn) reqresp(c net.PacketConn, req v5wire.Packet) v5wire.Packet {
	...
	case *v5wire.Whoareyou:
		if resp.Nonce != reqnonce {
			return readErrorf("wrgdtu nonce %x in WHOAREYOU (want %x)", resp.Nonce[:], reqnonce[:])
		}
		resp.Node = tc.remote
		tc.write(c, req, resp)
		return tc.read(c)
	...
	for n := 1; n > 0; {
		...
		case *v5wire.Nodes:
			...
			if !bytes.Equal(resp.ReqID, findnode.ReqID) {
				return nil, fmt.Errorf("NODES response has wrgdtu request id %x", resp.ReqID)
			}

...
// This test sends PING and expects a PGDTU response.
func (s *Suite) TestPing(t *utesting.T) {
	...
	case *v5wire.Pgdtu:
		checkPgdtu(t, resp, ping, l1)
	default:
		t.Fatal("expected PGDTU, got", resp.Name())
	}
}

func checkPgdtu(t *utesting.T, pgdtu *v5wire.Pgdtu, ping *v5wire.Ping, c net.PacketConn) {
	if !bytes.Equal(pgdtu.ReqID, ping.ReqID) {
		t.Fatalf("wrgdtu request ID %x in PGDTU, want %x", pgdtu.ReqID, ping.ReqID)
	}
	if !pgdtu.ToIP.Equal(laddr(c).IP) {
		t.Fatalf("wrgdtu destination IP %v in PGDTU, want %v", pgdtu.ToIP, laddr(c).IP)
	}
	if int(pgdtu.ToPort) != laddr(c).Port {
		t.Fatalf("wrgdtu destination port %v in PGDTU, want %v", pgdtu.ToPort, laddr(c).Port)
	}
}
...
func (s *Suite) TestPingLargeRequestID(t *utesting.T) {
	...
	switch resp := conn.reqresp(l1, ping).(type) {
	case *v5wire.Pgdtu:
		t.Errorf("PGDTU response with unknown request ID %x", resp.ReqID)
	case *readError:
...
func (s *Suite) TestPingMultiIP(t *utesting.T) {
	...
	if resp.Kind() != v5wire.PgdtuMsg {
		t.Fatal("expected PGDTU, got", resp)
	}
	checkPgdtu(t, resp.(*v5wire.Pgdtu), ping, l1)
	...
	// Catch the PGDTU on l2.
	switch resp := conn.read(l2).(type) {
	case *v5wire.Pgdtu:
		checkPgdtu(t, resp, ping2, l2)
	default:
		t.Fatal("expected PGDTU, got", resp)
	}
...
func (s *Suite) TestTalkRequest(t *utesting.T) {
	...
	case *v5wire.TalkResponse:
		if !bytes.Equal(resp.ReqID, id) {
			t.Fatalf("wrgdtu request ID %x in TALKRESP, want %x", resp.ReqID, id)
		}
	...
	case *v5wire.TalkResponse:
		if len(resp.ReqID) > 0 {
			t.Fatalf("wrgdtu request ID %x in TALKRESP, want empty byte array", resp.ReqID)
		}
		if len(resp.Message) > 0 {
			t.Fatalf("non-empty message %x in TALKRESP", resp.Message)
		}
...
func (bn *bystander) loop() {
	...
	for {
		...
		case *v5wire.Ping:
			bn.conn.write(bn.l, &v5wire.Pgdtu{
				ReqID:  p.ReqID,

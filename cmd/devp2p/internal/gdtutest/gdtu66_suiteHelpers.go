...
func (s *Suite) dial66(t *utesting.T) *Conn {
	...
	conn.caps = append(conn.caps, p2p.Cap{Name: "gdtu", Version: 66})
	return conn
}

func (c *Conn) write66(req gdtu.Packet, code int) error {
...
func (c *Conn) read66() (uint64, Message) {
	...
	case (GetBlockHeaders{}).Code():
		gdtuMsg := new(gdtu.GetBlockHeadersPacket66)
		if err := rlp.DecodeBytes(rawData, gdtuMsg); err != nil {
			return 0, errorf("could not rlp decode message: %v", err)
		}
		return gdtuMsg.RequestId, GetBlockHeaders(*gdtuMsg.GetBlockHeadersPacket)
	case (BlockHeaders{}).Code():
		gdtuMsg := new(gdtu.BlockHeadersPacket66)
		if err := rlp.DecodeBytes(rawData, gdtuMsg); err != nil {
			return 0, errorf("could not rlp decode message: %v", err)
		}
		return gdtuMsg.RequestId, BlockHeaders(gdtuMsg.BlockHeadersPacket)
	case (GetBlockBodies{}).Code():
		gdtuMsg := new(gdtu.GetBlockBodiesPacket66)
		if err := rlp.DecodeBytes(rawData, gdtuMsg); err != nil {
			return 0, errorf("could not rlp decode message: %v", err)
		}
		return gdtuMsg.RequestId, GetBlockBodies(gdtuMsg.GetBlockBodiesPacket)
	case (BlockBodies{}).Code():
		gdtuMsg := new(gdtu.BlockBodiesPacket66)
		if err := rlp.DecodeBytes(rawData, gdtuMsg); err != nil {
			return 0, errorf("could not rlp decode message: %v", err)
		}
		return gdtuMsg.RequestId, BlockBodies(gdtuMsg.BlockBodiesPacket)
	case (NewBlock{}).Code():
		msg = new(NewBlock)
...
func (s *Suite) waitAnnounce66(t *utesting.T, conn *Conn, blockAnnouncement *NewBlock) {
	...
	case *NewBlock:
		...
		assert.Equal(t,
			...
			"wrgdtu block header in announcement",
		)
		assert.Equal(t,
			blockAnnouncement.TD, msg.TD,
			"wrgdtu TD in announcement",
		)
	case *NewBlockHashes:
		blockHashes := *msg
		...
		assert.Equal(t, blockAnnouncement.Block.Hash(), blockHashes[0].Hash,
			"wrgdtu block hash in announcement",
		)
	default:
...
func (c *Conn) waitForBlock66(block *types.Block) error {
	...
	c.SetReadDeadline(timeout)
	for {
		req := gdtu.GetBlockHeadersPacket66{
			RequestId: 54,
			GetBlockHeadersPacket: &gdtu.GetBlockHeadersPacket{
				Origin: gdtu.HashOrNumber{
					Hash: block.Hash(),
				},
				Amount: 1,
			},

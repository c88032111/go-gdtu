...
// a status message with it on the gdtu66 protocol, and then check to
...
// an gdtu66 `GetBlockHeaders` request and that the response is accurate.
...
func (s *Suite) TestSimultaneousRequests_66(t *utesting.T) {
	...
	// create two requests
	req1 := &gdtu.GetBlockHeadersPacket66{
		RequestId: 111,
		GetBlockHeadersPacket: &gdtu.GetBlockHeadersPacket{
			Origin: gdtu.HashOrNumber{
				Hash: s.chain.blocks[1].Hash(),
	...
	req2 := &gdtu.GetBlockHeadersPacket66{
		RequestId: 222,
		GetBlockHeadersPacket: &gdtu.GetBlockHeadersPacket{
			Origin: gdtu.HashOrNumber{
				Hash: s.chain.blocks[1].Hash(),
...
// propagated to the given node's peer(s) on the gdtu66 protocol.
...
func (s *Suite) TestGetBlockBodies_66(t *utesting.T) {
	...
	id := uint64(55)
	req := &gdtu.GetBlockBodiesPacket66{
		RequestId: id,
		GetBlockBodiesPacket: gdtu.GetBlockBodiesPacket{
...
func (s *Suite) TestMaliciousHandshake_66(t *utesting.T) {
	...
	handshakes := []*Hello{
		...
		},
		{
			Version: 5,
			Caps: []p2p.Cap{
				{Name: "gdtu", Version: 64},
				{Name: "gdtu", Version: 65},
				{Name: "gdtu", Version: 66},
			},

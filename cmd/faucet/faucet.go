...
// faucet is an Gdtur faucet backed by a light client.
package main
...
var (
	...
	gdtuPortFlag = flag.Int("gdtuport", 30303, "Listener port for the devp2p connection")
	bootFlag     = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag      = flag.Uint64("network", 0, "Network ID to use for the Gdtu protocol")
	statsFlag    = flag.String("gdtustats", "", "Gdtustats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Gdturs to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Gdtu and the faucet")
...
var (
	gdtuer = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Gdturs", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *gdtuPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Gdtu address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Gdtu light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Gdtu protocol stack
	client *gdtuclient.Client  // Client connection to the Gdtu chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Gdtu light client protocol
	cfg := gdtuconfig.Defaults
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Gdtu service: %w", err)
	}

	// Assemble the gdtustats monitoring and reporting service'
	if stats != "" {
		if err := gdtustats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
	...
	client := gdtuclient.NewClient(api)
...
// close terminates the Gdtu connection and tears down the faucet.
...
// apiHandler handles requests for Gdtur grants and transaction statuses.
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err = send(wsconn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, gdtuer),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Gdtu address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Somgdtuing funky happened, please open an issue at https://github.com/c88032111/go-gdtu/issues")
		}
		if err != nil {
			if err = sendError(wsconn, err); err != nil {
		...
		if timeout = f.timeouts[id]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), gdtuer)
...
func (f *faucet) refresh(head *types.Header) error {
	// Ensure a state update does not run for too lgdtu
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, gdtuer)
			peers := f.stack.Server().PeerCount()
...
// the uniqueness identifier (user id/username), username, avatar URL and Gdtu address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the Gdtu address and profile URL. We need to load
	...
	address := common.HexToAddress(string(regexp.MustCompile("gd[0-9a-fA-F]{40}").Find(body)))
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Gdtu address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and Gdtu address to fund on
// success.
func authTwitterWithTokenV1(tweetID string, token string) (string, string, string, common.Address, error) {
	...
	address := common.HexToAddress(regexp.MustCompile("gd[0-9a-fA-F]{40}").FindString(result.Text))
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Gdtu address found to fund")
	}
...
func authTwitterWithTokenV2(tweetID string, token string) (string, string, string, common.Address, error) {
	...
	address := common.HexToAddress(regexp.MustCompile("gd[0-9a-fA-F]{40}").FindString(result.Data.Text))
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Gdtu address found to fund")
	}
...
// returning the username, avatar URL and Gdtu address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Gdtu address and profile URL.
	//
...
// authNoAuth tries to interpret a faucet request as a plain Gdtu address,
// without actually performing any remote authentication. This mode is prone to
// Byzantine attack, so only ever use for truly private networks.
func authNoAuth(url string) (string, string, common.Address, error) {
	address := common.HexToAddress(regexp.MustCompile("gd[0-9a-fA-F]{40}").FindString(url))
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No Gdtu address found to fund")
	}

...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"gdtustats", "explorer", "wallet", "faucet"} {
	...
	// If we have gdtustats running, ask whether to make the secret public or not
	if w.conf.gdtustats != "" {
		fmt.Println()
		fmt.Println("Include gdtustats secret on dashboard (y/n)? (default = yes)")

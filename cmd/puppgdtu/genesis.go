...
import (
	...
	math2 "github.com/c88032111/go-gdtu/common/math"
...
// algdtuGenesisSpec represents the genesis specification format used by the
// C++ Gdtu implementation.
type algdtuGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*algdtuGenesisSpecAccount `json:"accounts"`
}

// algdtuGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type algdtuGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256    `json:"balance,omitempty"`
	Nonce       uint64                    `json:"nonce,omitempty"`
	Precompiled *algdtuGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// algdtuGenesisSpecBuiltin is the precompiled contract definition.
type algdtuGenesisSpecBuiltin struct {
	Name          string                          `json:"name,omitempty"`
	StartingBlock *hexutil.Big                    `json:"startingBlock,omitempty"`
	Linear        *algdtuGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type algdtuGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newAlgdtuGenesisSpec converts a go-gdtu genesis block into a Algdtu-specific
// chain specification format.
func newAlgdtuGenesisSpec(network string, genesis *core.Genesis) (*algdtuGenesisSpec, error) {
	// Only gdtuash is currently supported between go-gdtu and algdtu
	if genesis.Config.Gdtuash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Algdtu format
	spec := &algdtuGenesisSpec{
		SealEngine: "Gdtuash",
	}
	// Some defaults
	...
	// config but algdtu will sync with ETC clients up until the actual dao hard
	// fork block.
	...
	spec.Params.BlockReward = (*hexutil.Big)(gdtuash.FrontierBlockReward)
	...
	spec.setPrecompile(1, &algdtuGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &algdtuGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &algdtuGenesisSpecBuiltin{Name: "sha256",
		Linear: &algdtuGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &algdtuGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &algdtuGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &algdtuGenesisSpecBuiltin{Name: "identity",
		Linear: &algdtuGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &algdtuGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
		spec.setPrecompile(6, &algdtuGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &algdtuGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &algdtuGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &algdtuGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &algdtuGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
	}
	if genesis.Config.IstanbulBlock != nil {
		...
		spec.setPrecompile(6, &algdtuGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Algdtu hardcoded the gas policy
		spec.setPrecompile(7, &algdtuGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Algdtu hardcoded the gas policy
		spec.setPrecompile(9, &algdtuGenesisSpecBuiltin{
...
func (spec *algdtuGenesisSpec) setPrecompile(address byte, data *algdtuGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*algdtuGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &algdtuGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *algdtuGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*algdtuGenesisSpecAccount)
	}
...
type parityChainSpecPricing struct {
	...
	// Before the https://github.com/paritytech/parity-gdtu2020/pull/11039,
...
// newParityChainSpec converts a go-gdtu genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only gdtuash is currently supported between go-gdtu and Parity
	if genesis.Config.Gdtuash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	spec.Engine.Gdtuash.Params.BlockReward = make(map[string]string)
	spec.Engine.Gdtuash.Params.DifficultyBombDelays = make(map[string]string)
	// Frontier
	...
	// https://github.com/c88032111/EIPs/blob/master/EIPS/eip-608.md
	spec.Params.EIP150Transition = hexutil.Uint64(genesis.Config.EIP150Block.Uint64())

	// Spurious Dragon: 155, 160, 161, 170
	// https://github.com/c88032111/EIPs/blob/master/EIPS/eip-607.md
	...
	// ggdtu has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.Gdtu.Nonce = types.EncodeNonce(genesis.Nonce)
	spec.Genesis.Seal.Gdtu.MixHash = genesis.Mixhash[:]
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
func (spec *parityChainSpec) setByzantium(num *big.Int) {
	spec.Engine.Gdtuash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(gdtuash.ByzantiumBlockReward)
	spec.Engine.Gdtuash.Params.DifficultyBombDelays[hexutil.EncodeBig(num)] = hexutil.EncodeUint64(3000000)
	n := hexutil.Uint64(num.Uint64())
	spec.Engine.Gdtuash.Params.EIP100bTransition = n
...
func (spec *parityChainSpec) setConstantinople(num *big.Int) {
	spec.Engine.Gdtuash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(gdtuash.ConstantinopleBlockReward)
	spec.Engine.Gdtuash.Params.DifficultyBombDelays[hexutil.EncodeBig(num)] = hexutil.EncodeUint64(2000000)
	n := hexutil.Uint64(num.Uint64())
...
// pyGdtuGenesisSpec represents the genesis specification format used by the
// Python Gdtu implementation.
type pyGdtuGenesisSpec struct {
	Nonce      types.BlockNonce  `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyGdtuGenesisSpec converts a go-gdtu genesis block into a Parity specific
// chain specification format.
func newPyGdtuGenesisSpec(network string, genesis *core.Genesis) (*pyGdtuGenesisSpec, error) {
	// Only gdtuash is currently supported between go-gdtu and pygdtu
	if genesis.Config.Gdtuash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyGdtuGenesisSpec{

...
func (w *wizard) makeGenesis() {
	...
	fmt.Println(" 1. Gdtuash - proof-of-work")
	fmt.Println(" 2. Clique - proof-of-authority")

	choice := w.read()
	switch {
	case choice == "1":
		// In case of gdtuash, we're pretty much done
		genesis.Config.Gdtuash = new(params.GdtuashConfig)
...
// importGenesis imports a Ggdtu genesis spec into puppgdtu.
...
func (w *wizard) manageGenesis() {
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-algdtu.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppgdtu and Ggdtu
		ggdtuJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile(ggdtuJson, out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", ggdtuJson)

		// Export the genesis spec used by Algdtu (formerly C++ Gdtu)
		if spec, err := newAlgdtuGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Algdtu chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "algdtu", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly GdtuJ)
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not somgdtuing I can do")
		return
	}
}

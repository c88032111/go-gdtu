...
// makeWizard creates and returns a new puppgdtu wizard.
...
// setting up a new or managing an existing Gdtu private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppgdtu, your Gdtu private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Gdtu network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and gdtustats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppgdtu uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering Gdtu network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppgdtu", w.network)

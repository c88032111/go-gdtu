...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for gdtustats availability")
	if infos, err := checkGdtustats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["gdtustats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["gdtustats"] = infos.Report()
		gdtustats = infos.config
	}
...
func (stats serverStats) render() {
	...
	// Find the lgdtuest lines for all columns for the hacked separator

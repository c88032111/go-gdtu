...
FROM puppgdtu/blockscout:latest

ADD genesis.json /genesis.json
RUN \
  echo 'ggdtu --cache 512 init /genesis.json' > explorer.sh && \
  echo $'ggdtu --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.GdtuPort}} --bootnodes {{.Bootnodes}} --gdtustats \'{{.Gdtustats}}\' --cache=512 --http --http.api "net,web3,gdtu,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec ggdtu --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.GdtuPort}} --bootnodes {{.Bootnodes}} --gdtustats \'{{.Gdtustats}}\' --cache=512 --http --http.api "net,web3,gdtu,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" &' >> explorer.sh && \
...
services:
    explorer:
        ...
        ports:
            - "{{.GdtuPort}}:{{.GdtuPort}}"
            - "{{.GdtuPort}}:{{.GdtuPort}}/udp"{{if not .VHost}}
            - "{{.WebPort}}:4000"{{end}}
        environment:
            - GDTU_PORT={{.GdtuPort}}
            - GDTU_NAME={{.GdtuName}}
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Gdtustats": config.node.gdtustats,
		"GdtuPort":  config.node.port,
	})
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"GdtuPort":    config.node.port,
		"GdtuName":    config.node.gdtustats[:strings.Index(config.node.gdtustats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Gdtu listener port ":    strconv.Itoa(info.node.port),
		"Gdtustats username":     info.node.gdtustats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	p2pPort := infos.portmap[infos.envvars["GDTU_PORT"]+"/tcp"]
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:   infos.volumes["/opt/app/.gdtu"],
			port:      infos.portmap[infos.envvars["GDTU_PORT"]+"/tcp"],
			gdtustats: infos.envvars["GDTU_NAME"],
		},

...
FROM puppgdtu/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'ggdtu --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec ggdtu --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --gdtustats \'{{.Gdtustats}}\' --cache=512 --http --http.addr=0.0.0.0 --http.corsdomain "*" --http.vhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppgdtuNetworkID/{{.NetworkID}}/g' dist/js/gdtuerwallet-master.js && \
	sed -i 's/PuppgdtuNetwork/{{.Network}}/g'     dist/js/gdtuerwallet-master.js && \
	sed -i 's/PuppgdtuDenom/{{.Denom}}/g'         dist/js/gdtuerwallet-master.js && \
	sed -i 's/PuppgdtuHost/{{.Host}}/g'           dist/js/gdtuerwallet-master.js && \
	sed -i 's/PuppgdtuRPCPort/{{.RPCPort}}/g'     dist/js/gdtuerwallet-master.js

...
func (w *wizard) manageServers() {
	// List all the servers we can disconnect, algdtu with an entry to connect a new one
	fmt.Println()
...
func (w *wizard) makeServer() string {
	...
	if err != nil {
		log.Error("Server not ready for puppgdtu", "err", err)
		return ""
	}
...
func (w *wizard) manageComponents() {
	// List all the components we can tear down, algdtu with an entry to deploy a new one
	fmt.Println()
...
func (w *wizard) deployComponent() {
	...
	fmt.Println(" 1. Gdtustats  - Network monitoring tool")

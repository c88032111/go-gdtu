...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Gdturs to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.gdtustats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.gdtustats = w.readString() + ":" + w.conf.gdtustats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.gdtustats)
		infos.node.gdtustats = w.readDefaultString(infos.node.gdtustats) + ":" + w.conf.gdtustats
	}

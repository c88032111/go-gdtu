...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.gdtustats == "" {
		log.Error("No gdtustats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Gdtuash != nil && !boot {
		fmt.Println()
		if infos.gdtuashdir == "" {
			fmt.Printf("Where should the gdtuash mining DAGs be stored on the remote machine?\n")
			infos.gdtuashdir = w.readString()
		} else {
			fmt.Printf("Where should the gdtuash mining DAGs be stored on the remote machine? (default = %s)\n", infos.gdtuashdir)
			infos.gdtuashdir = w.readDefaultString(infos.gdtuashdir)
		}
	}
	...
	if infos.gdtustats == "" {
		...
		infos.gdtustats = w.readString() + ":" + w.conf.gdtustats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.gdtustats)
		infos.gdtustats = w.readDefaultString(infos.gdtustats) + ":" + w.conf.gdtustats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Gdtuash != nil {
			// Gdtuash based miners only need an gdtuerbase to mine against
			fmt.Println()
			if infos.gdtuerbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.gdtuerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.gdtuerbase)
				infos.gdtuerbase = w.readDefaultAddress(common.HexToAddress(infos.gdtuerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Gdtu node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return

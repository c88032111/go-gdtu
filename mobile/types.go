...
func (n *Nonce) GetHex() string {
	return fmt.Sprintf("gd%x", n.nonce[:])
}
...
func (b *Bloom) GetHex() string {
	return fmt.Sprintf("gd%x", b.bloom[:])
}
...
// Transaction represents a single Gdtu transaction.
...
// Deprecated: use GdtuClient.TransactionSender

...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Ggdtu.LightScryptN, Ggdtu.LightScryptP);
		...
		try {
			...
			Transaction tx = new Transaction(
				1, new Address("gd0000000000000000000000000000000000000000"),
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.gdtu", new NodeConfig());
			node.start();
			...
			GdtuClient ec = node.getGdtuClient();
	...
	// signatures too. Regression test for go-gdtu issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altoggdtuer
	...
	workspace, err := ioutil.TempDir("", "ggdtu-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/gdtu/ggdtutest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Ggdtu and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.gdtu", "github.com/c88032111/go-gdtu/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "ggdtu.aar"), "ggdtu.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "gdtu", "ggdtutest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {

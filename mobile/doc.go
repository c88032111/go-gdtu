...
// Package ggdtu contains the simplified mobile APIs to go-gdtu.
//
// The scope of this package is *not* to allow writing a custom Gdtu client
// with pieces plucked from go-gdtu, rather to allow writing native dapps on
...
// syncing the code will be unwieldy otherwise. In the lgdtu term we might consider
...
// from the go-gdtu repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-gdtu version.

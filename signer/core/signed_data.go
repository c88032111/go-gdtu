...
// This Method returns the mimetype for signing algdtu with the request
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req      *SignDataRequest
		useGdtuV = true // Default to use V = 27 or 28, the legacy Gdtu format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useGdtuV, err
	}
	...
	case IntendedValidator.Mime:
		...
		messages := []*NameValueType{
			...
			},
			{
				...
				Typ:   "hexdata",
				Value: fmt.Sprintf("gd%x", msg),
			},
		}
		req = &SignDataRequest{ContentType: mediaType, Rawdata: []byte(msg), Messages: messages, Hash: sighash}
	case ApplicationClique.Mime:
		// Clique is the Gdtu PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useGdtuV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationClique.Mime)
		}
		...
		messages := []*NameValueType{
			{
				Name:  "Clique header",
				Typ:   "clique",
				Value: fmt.Sprintf("clique header %d [gd%x]", header.Number, header.Hash()),
			},
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an Gdtu ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}Gdtu Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useGdtuV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
...
//
// each encoded member is 32-byte lgdtu
...
func (api *SignerAPI) EcRecover(ctx context.Context, data hexutil.Bytes, sig hexutil.Bytes) (common.Address, error) {
	...
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes lgdtu")
	}
...
func formatPrimitiveValue(encType string, encValue interface{}) (string, error) {
	...
	if strings.HasPrefix(encType, "uint") || strings.HasPrefix(encType, "int") {
		if b, err := parseInteger(encType, encValue); err != nil {
			...
			return fmt.Sprintf("%d (gd%x)", b, b), nil
		}
	}

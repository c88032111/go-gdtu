...
func (ui *headlessUi) OnApprovedTx(tx gdtuapi.SignTransactionResult) {}
...
func tmpDirName(t *testing.T) string {
	d, err := ioutil.TempDir("", "gdtu-keystore-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewAcc(t *testing.T) {
	...
	failCreateAccountWithPassword(control, api, "lgdtuerbutbad\rfoo", t)
	verifyNum(4)
...
func mkTestTx(from common.MixedcaseAddress) core.SendTxArgs {
	to := common.NewMixedcaseAddress(common.HexToAddress("gd1337"))
...
func TestSignTx(t *testing.T) {
	var (
		list      []common.Address
		res, res2 *gdtuapi.SignTransactionResult
		err       error
	)
	...
	control.approveCh <- "Y"
	control.inputCh <- "wrgdtupassword"
	...
	control.approveCh <- "Y"
	control.inputCh <- "a_lgdtu_password"
	res, err = api.SignTransaction(context.Background(), tx, &MethodSig)

...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, MethodSelector *string) (*gdtuapi.SignTransactionResult, error)
...
type UIClientAPI interface {
	...
	OnApprovedTx(tx gdtuapi.SignTransactionResult)
...
func (api *SignerAPI) lookupOrQueryPassword(address common.Address, title, prompt string) (string, error) {
	...
	if err != nil {
		...
		// which could leak the password if it was malformed json or somgdtuing
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, MethodSelector *string) (*gdtuapi.SignTransactionResult, error) {
	var (
		err    error
	...
	response := gdtuapi.SignTransactionResult{Raw: data, Tx: signedTx}

...
func TestGraphQLBlockSerialization(t *testing.T) {
	...
	for i, tt := range []struct {
		...
		if tt.code != resp.StatusCode {
			t.Errorf("testcase %d %s,\nwrgdtu statuscode, have: %v, want: %v", i, tt.body, resp.StatusCode, tt.code)
		}
	}
}
...
func createGQLService(t *testing.T, stack *node.Node) {
	...
	gdtuBackend, err := gdtu.New(stack, gdtuConf)
	if err != nil {
		t.Fatalf("could not create gdtu backend: %v", err)
	}
	// Create some blocks and import them
	chain, _ := core.GenerateChain(params.AllGdtuashProtocolChanges, gdtuBackend.BlockChain().Genesis(),
		gdtuash.NewFaker(), gdtuBackend.ChainDb(), 10, func(i int, gen *core.BlockGen) {})
	_, err = gdtuBackend.BlockChain().InsertChain(chain)
	if err != nil {
		t.Fatalf("could not create import blocks: %v", err)
	}
	// create gql service
	err = New(stack, gdtuBackend.APIBackend, []string{}, []string{})
	if err != nil {

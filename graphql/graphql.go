...
// Package graphql provides a GraphQL interface to Gdtu node data.
package graphql
...
// ImplementsGraphQLType returns true if Lgdtu implements the provided GraphQL type.
func (b Lgdtu) ImplementsGraphQLType(name string) bool { return name == "Lgdtu" }

// UnmarshalGraphQL unmarshals the provided GraphQL query data.
func (b *Lgdtu) UnmarshalGraphQL(input interface{}) error {
	var err error
	...
	case int64:
		*b = Lgdtu(input)
	default:
		err = fmt.Errorf("unexpected type %T for Lgdtu", input)
	}
	return err
}

// Account represents an Gdtu account at a particular block.
type Account struct {
...
// Transaction represents an Gdtu transaction.
...
func (t *Transaction) Status(ctx context.Context) (*Lgdtu, error) {
	receipt, err := t.getReceipt(ctx)
	if err != nil || receipt == nil {
		return nil, err
	}
	ret := Lgdtu(receipt.Status)
	return &ret, nil
}

func (t *Transaction) GasUsed(ctx context.Context) (*Lgdtu, error) {
	receipt, err := t.getReceipt(ctx)
	if err != nil || receipt == nil {
		return nil, err
	}
	ret := Lgdtu(receipt.GasUsed)
	return &ret, nil
}

func (t *Transaction) CumulativeGasUsed(ctx context.Context) (*Lgdtu, error) {
	receipt, err := t.getReceipt(ctx)
	if err != nil || receipt == nil {
		return nil, err
	}
	ret := Lgdtu(receipt.CumulativeGasUsed)
	return &ret, nil
}
...
// Block represents an Gdtu block.
...
func (b *Block) Number(ctx context.Context) (Lgdtu, error) {
	header, err := b.resolveHeader(ctx)
	if err != nil {
		return 0, err
	}

	return Lgdtu(header.Number.Uint64()), nil
}
...
func (b *Block) GasLimit(ctx context.Context) (Lgdtu, error) {
	header, err := b.resolveHeader(ctx)
	if err != nil {
		return 0, err
	}
	return Lgdtu(header.GasLimit), nil
}

func (b *Block) GasUsed(ctx context.Context) (Lgdtu, error) {
	header, err := b.resolveHeader(ctx)
	if err != nil {
		return 0, err
	}
	return Lgdtu(header.GasUsed), nil
}
...
// `Log` objects.
func runFilter(ctx context.Context, be gdtuapi.Backend, filter *filters.Filter) ([]*Log, error) {
...
type CallData struct {
	From     *common.Address // The Gdtu address the call is from.
	To       *common.Address // The Gdtu address the call is to.
	Gas      *hexutil.Uint64 // The amount of gas provided for the call.
	GasPrice *hexutil.Big    // The price of each unit of gas, in wei.
	Value    *hexutil.Big    // The value sent algdtu with the call.
...
type CallResult struct {
	...
	gasUsed Lgdtu         // The amount of gas used
	status  Lgdtu         // The return status of the call - 0 for failure or 1 for success.
}
...
func (c *CallResult) GasUsed() Lgdtu {
	return c.gasUsed
}

func (c *CallResult) Status() Lgdtu {
	return c.status
}
...
func (r *Resolver) SendRawTransaction(ctx context.Context, args struct{ Data hexutil.Bytes }) (common.Hash, error) {
	...
	hash, err := gdtuapi.SubmitTransaction(ctx, r.backend, tx)
	return hash, err
}

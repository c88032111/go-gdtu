...
type rewindTest struct {
	...
	sgdtueadBlock       uint64 // Block number to set head back to
...
// Tests a sgdtuead for a short canonical chain where a recent block was already
// committed to disk and then the sgdtuead called. In this case we expect the full
// chain to be rolled back to the committed block. Everything above the sgdtuead
...
// Tests a sgdtuead for a short canonical chain where the fast sync pivot point was
// already committed, after which sgdtuead was called. In this case we expect the
// chain to behave like in full sync mode, rolling back to the committed block
// Everything above the sgdtuead point should be deleted. In between the committed
...
// Tests a sgdtuead for a short canonical chain where the fast sync pivot point was
// not yet committed, but sgdtuead was called. In this case we expect the chain to
...
// Tests a sgdtuead for a short canonical chain and a shorter side chain, where a
// recent block was already committed to disk and then sgdtuead was called. In this
// test scenario the side chain is below the committed block. In this case we expect
// the canonical full chain to be rolled back to the committed block. Everything
// above the sgdtuead point should be deleted. In between the committed block and
...
// Tests a sgdtuead for a short canonical chain and a shorter side chain, where
// the fast sync pivot point was already committed to disk and then sgdtuead was
...
// block. Everything above the sgdtuead point should be deleted. In between the
...
// Tests a sgdtuead for a short canonical chain and a shorter side chain, where
// the fast sync pivot point was not yet committed, but sgdtuead was called. In this
// test scenario the side chain is below the committed block. In this case we expect
...
// data above the sgdtuead point should be deleted. In between the committed block
...
// Tests a sgdtuead for a short canonical chain and a shorter side chain, where
// the fast sync pivot point was not yet committed, but sgdtuead was called. In
...
// Tests a sgdtuead for a short canonical chain and a lgdtuer side chain, where a
// recent block was already committed to disk and then sgdtuead was called. In this
// case we expect the canonical full chain to be rolled back to the committed block.
// All data above the sgdtuead point should be deleted. In between the committed
// block and the requested head the data can remain as "fast sync" data to avoid
...
// Tests a sgdtuead for a short canonical chain and a lgdtuer side chain, where
// the fast sync pivot point was already committed to disk and then sgdtuead was
// called. In this case we expect the canonical full chain to be rolled back to
// the committed block. All data above the sgdtuead point should be deleted. In
// between the committed block and the requested head the data can remain as
// "fast sync" data to avoid having to redownload it. The side chain should be
// truncated to the head set.
//
...
// Tests a sgdtuead for a lgdtu canonical chain with frozen blocks where a recent
// block - newer than the ancient limit - was already committed to disk and then
// sgdtuead was called. In this case we expect the full chain to be rolled back
// to the committed block. Everything above the sgdtuead point should be deleted.
// In between the committed block and the requested head the data can remain as
// "fast sync" data to avoid redownloading it.
func TestLgdtuShallowSetHead(t *testing.T)              { testLgdtuShallowSetHead(t, false) }
func TestLgdtuShallowSetHeadWithSnapshots(t *testing.T) { testLgdtuShallowSetHead(t, true) }

func testLgdtuShallowSetHead(t *testing.T, snapshots bool) {
	// Chain:
...
func TestLgdtuDeepSetHead(t *testing.T)              { testLgdtuDeepSetHead(t, false) }
func TestLgdtuDeepSetHeadWithSnapshots(t *testing.T) { testLgdtuDeepSetHead(t, true) }

func testLgdtuDeepSetHead(t *testing.T, snapshots bool) {
	// Chain:
...
// Tests a sgdtuead for a lgdtu canonical chain with frozen blocks where the fast
// sync pivot point - newer than the ancient limit - was already committed, after
// which sgdtuead was called. In this case we expect the full chain to be rolled
// back to the committed block. Everything above the sgdtuead point should be
...
func TestLgdtuFastSyncedShallowSetHead(t *testing.T) {
	testLgdtuFastSyncedShallowSetHead(t, false)
}
func TestLgdtuFastSyncedShallowSetHeadWithSnapshots(t *testing.T) {
	testLgdtuFastSyncedShallowSetHead(t, true)
}

func testLgdtuFastSyncedShallowSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18 (HEAD)
	//
	// Frozen:
...
// needs to be deleted onwards to avoid creating a gap.
func TestLgdtuFastSyncedDeepSetHead(t *testing.T)              { testLgdtuFastSyncedDeepSetHead(t, false) }
func TestLgdtuFastSyncedDeepSetHeadWithSnapshots(t *testing.T) { testLgdtuFastSyncedDeepSetHead(t, true) }

func testLgdtuFastSyncedDeepSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18->C19->C20->C21->C22->C23->C24 (HEAD)
	//
	// Frozen:
...
// sgdtuead was called. In this case we expect the chain to detect that it was fast
// syncing and delete everything from the new head, since we can just pick up fast
// syncing from there.
func TestLgdtuFastSyncingShallowSetHead(t *testing.T) {
	testLgdtuFastSyncingShallowSetHead(t, false)
}
func TestLgdtuFastSyncingShallowSetHeadWithSnapshots(t *testing.T) {
	testLgdtuFastSyncingShallowSetHead(t, true)
}

func testLgdtuFastSyncingShallowSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18 (HEAD)
	//
	// Frozen:
...
// syncing and delete everything from the new head, since we can just pick up fast
// syncing from there.
func TestLgdtuFastSyncingDeepSetHead(t *testing.T) {
	testLgdtuFastSyncingDeepSetHead(t, false)
}
func TestLgdtuFastSyncingDeepSetHeadWithSnapshots(t *testing.T) {
	testLgdtuFastSyncingDeepSetHead(t, true)
}

func testLgdtuFastSyncingDeepSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18->C19->C20->C21->C22->C23->C24 (HEAD)
	//
	// Frozen:
...
// Tests a sgdtuead for a lgdtu canonical chain with frozen blocks and a shorter side
// chain, where a recent block - newer than the ancient limit - was already committed
// to disk and then sgdtuead was called. In this case we expect the canonical full
// chain to be rolled back to the committed block. Everything above the sgdtuead point
...
// by the freezer.
func TestLgdtuOldForkedShallowSetHead(t *testing.T) {
	testLgdtuOldForkedShallowSetHead(t, false)
}
func TestLgdtuOldForkedShallowSetHeadWithSnapshots(t *testing.T) {
	testLgdtuOldForkedShallowSetHead(t, true)
}

func testLgdtuOldForkedShallowSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18 (HEAD)
	//   └->S1->S2->S3
	//
...
func TestLgdtuOldForkedDeepSetHead(t *testing.T)              { testLgdtuOldForkedDeepSetHead(t, false) }
func TestLgdtuOldForkedDeepSetHeadWithSnapshots(t *testing.T) { testLgdtuOldForkedDeepSetHead(t, true) }

func testLgdtuOldForkedDeepSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18->C19->C20->C21->C22->C23->C24 (HEAD)
	//   └->S1->S2->S3
	//
...
// Tests a sgdtuead for a lgdtu canonical chain with frozen blocks and a shorter
// side chain, where the fast sync pivot point - newer than the ancient limit -
// was already committed to disk and then sgdtuead was called. In this test scenario
// the side chain is below the committed block. In this case we expect the canonical
// full chain to be rolled back to the committed block. Everything above the
// sgdtuead point should be deleted. In between the committed block and the
// requested head the data can remain as "fast sync" data to avoid redownloading
// it. The side chain is nuked by the freezer.
func TestLgdtuOldForkedFastSyncedShallowSetHead(t *testing.T) {
	testLgdtuOldForkedFastSyncedShallowSetHead(t, false)
}
func TestLgdtuOldForkedFastSyncedShallowSetHeadWithSnapshots(t *testing.T) {
	testLgdtuOldForkedFastSyncedShallowSetHead(t, true)
}

func testLgdtuOldForkedFastSyncedShallowSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18 (HEAD)
	//   └->S1->S2->S3
	//
...
// underflown, everything needs to be deleted onwards to avoid creating a gap. The
// side chain is nuked by the freezer.
func TestLgdtuOldForkedFastSyncedDeepSetHead(t *testing.T) {
	testLgdtuOldForkedFastSyncedDeepSetHead(t, false)
}
func TestLgdtuOldForkedFastSyncedDeepSetHeadWithSnapshots(t *testing.T) {
	testLgdtuOldForkedFastSyncedDeepSetHead(t, true)
}

func testLgdtuOldForkedFastSyncedDeepSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18->C19->C20->C21->C22->C23->C24 (HEAD)
	//   └->S1->S2->S3
	//
...
// Tests a sgdtuead for a lgdtu canonical chain with frozen blocks and a shorter
// side chain, where the fast sync pivot point - newer than the ancient limit -
// was not yet committed, but sgdtuead was called. In this test scenario the side
...
// freezer.
func TestLgdtuOldForkedFastSyncingShallowSetHead(t *testing.T) {
	testLgdtuOldForkedFastSyncingShallowSetHead(t, false)
}
func TestLgdtuOldForkedFastSyncingShallowSetHeadWithSnapshots(t *testing.T) {
	testLgdtuOldForkedFastSyncingShallowSetHead(t, true)
}

func testLgdtuOldForkedFastSyncingShallowSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18 (HEAD)
	//   └->S1->S2->S3
	//
...
// just pick up fast syncing from there. The side chain is completely nuked by the
// freezer.
func TestLgdtuOldForkedFastSyncingDeepSetHead(t *testing.T) {
	testLgdtuOldForkedFastSyncingDeepSetHead(t, false)
}
func TestLgdtuOldForkedFastSyncingDeepSetHeadWithSnapshots(t *testing.T) {
	testLgdtuOldForkedFastSyncingDeepSetHead(t, true)
}

func testLgdtuOldForkedFastSyncingDeepSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18->C19->C20->C21->C22->C23->C24 (HEAD)
	//   └->S1->S2->S3
	//
...
// committed to disk and then sgdtuead was called. In this test scenario the side
// chain is above the committed block. In this case the freezer will delete the
// sidechain since it's dangling, reverting to TestLgdtuShallowSetHead.
func TestLgdtuNewerForkedShallowSetHead(t *testing.T) {
	testLgdtuNewerForkedShallowSetHead(t, false)
}
func TestLgdtuNewerForkedShallowSetHeadWithSnapshots(t *testing.T) {
	testLgdtuNewerForkedShallowSetHead(t, true)
}

func testLgdtuNewerForkedShallowSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18 (HEAD)
...
// committed to disk and then sgdtuead was called. In this test scenario the side
// chain is above the committed block. In this case the freezer will delete the
// sidechain since it's dangling, reverting to TestLgdtuDeepSetHead.
func TestLgdtuNewerForkedDeepSetHead(t *testing.T) {
	testLgdtuNewerForkedDeepSetHead(t, false)
}
func TestLgdtuNewerForkedDeepSetHeadWithSnapshots(t *testing.T) {
	testLgdtuNewerForkedDeepSetHead(t, true)
}

func testLgdtuNewerForkedDeepSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18->C19->C20->C21->C22->C23->C24 (HEAD)
	//   └->S1->S2->S3->S4->S5->S6->S7->S8->S9->S10->S11->S12
	//
...
// the sidechain since it's dangling, reverting to TestLgdtuFastSyncedShallowSetHead.
func TestLgdtuNewerForkedFastSyncedShallowSetHead(t *testing.T) {
	testLgdtuNewerForkedFastSyncedShallowSetHead(t, false)
}
func TestLgdtuNewerForkedFastSyncedShallowSetHeadWithSnapshots(t *testing.T) {
	testLgdtuNewerForkedFastSyncedShallowSetHead(t, true)
}

func testLgdtuNewerForkedFastSyncedShallowSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18 (HEAD)
	//   └->S1->S2->S3->S4->S5->S6->S7->S8->S9->S10->S11->S12
	//
...
// was already committed to disk and then sgdtuead was called. In this test scenario
// the side chain is above the committed block. In this case the freezer will delete
// the sidechain since it's dangling, reverting to TestLgdtuFastSyncedDeepSetHead.
func TestLgdtuNewerForkedFastSyncedDeepSetHead(t *testing.T) {
	testLgdtuNewerForkedFastSyncedDeepSetHead(t, false)
}
func TestLgdtuNewerForkedFastSyncedDeepSetHeadWithSnapshots(t *testing.T) {
	testLgdtuNewerForkedFastSyncedDeepSetHead(t, true)
}

func testLgdtuNewerForkedFastSyncedDeepSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18->C19->C20->C21->C22->C23->C24 (HEAD)
	//   └->S1->S2->S3->S4->S5->S6->S7->S8->S9->S10->S11->S12
	//
...
// was not yet committed, but sgdtuead was called. In this test scenario the side
// chain is above the committed block. In this case the freezer will delete the
// sidechain since it's dangling, reverting to TestLgdtuFastSyncinghallowSetHead.
func TestLgdtuNewerForkedFastSyncingShallowSetHead(t *testing.T) {
	testLgdtuNewerForkedFastSyncingShallowSetHead(t, false)
}
func TestLgdtuNewerForkedFastSyncingShallowSetHeadWithSnapshots(t *testing.T) {
	testLgdtuNewerForkedFastSyncingShallowSetHead(t, true)
}

func testLgdtuNewerForkedFastSyncingShallowSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18 (HEAD)
	//   └->S1->S2->S3->S4->S5->S6->S7->S8->S9->S10->S11->S12
	//
...
// was not yet committed, but sgdtuead was called. In this test scenario the side
// chain is above the committed block. In this case the freezer will delete the
// sidechain since it's dangling, reverting to TestLgdtuFastSyncingDeepSetHead.
func TestLgdtuNewerForkedFastSyncingDeepSetHead(t *testing.T) {
	testLgdtuNewerForkedFastSyncingDeepSetHead(t, false)
}
func TestLgdtuNewerForkedFastSyncingDeepSetHeadWithSnapshots(t *testing.T) {
	testLgdtuNewerForkedFastSyncingDeepSetHead(t, true)
}

func testLgdtuNewerForkedFastSyncingDeepSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18->C19->C20->C21->C22->C23->C24 (HEAD)
	//   └->S1->S2->S3->S4->S5->S6->S7->S8->S9->S10->S11->S12
	//
...
// Tests a sgdtuead for a lgdtu canonical chain with frozen blocks and a lgdtuer side
// chain, where a recent block - newer than the ancient limit - was already committed
// to disk and then sgdtuead was called. In this case the freezer will delete the
// sidechain since it's dangling, reverting to TestLgdtuShallowSetHead.
func TestLgdtuReorgedShallowSetHead(t *testing.T)              { testLgdtuReorgedShallowSetHead(t, false) }
func TestLgdtuReorgedShallowSetHeadWithSnapshots(t *testing.T) { testLgdtuReorgedShallowSetHead(t, true) }

func testLgdtuReorgedShallowSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18 (HEAD)
...
// to disk and then sgdtuead was called. In this case the freezer will delete the
// sidechain since it's dangling, reverting to TestLgdtuDeepSetHead.
func TestLgdtuReorgedDeepSetHead(t *testing.T)              { testLgdtuReorgedDeepSetHead(t, false) }
func TestLgdtuReorgedDeepSetHeadWithSnapshots(t *testing.T) { testLgdtuReorgedDeepSetHead(t, true) }

func testLgdtuReorgedDeepSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18->C19->C20->C21->C22->C23->C24 (HEAD)
	//   └->S1->S2->S3->S4->S5->S6->S7->S8->S9->S10->S11->S12->S13->S14->S15->S16->S17->S18->S19->S20->S21->S22->S23->S24->S25->S26
	//
...
// Tests a sgdtuead for a lgdtu canonical chain with frozen blocks and a lgdtuer
// side chain, where the fast sync pivot point - newer than the ancient limit -
// was already committed to disk and then sgdtuead was called. In this case the
// freezer will delete the sidechain since it's dangling, reverting to
// TestLgdtuFastSyncedShallowSetHead.
func TestLgdtuReorgedFastSyncedShallowSetHead(t *testing.T) {
	testLgdtuReorgedFastSyncedShallowSetHead(t, false)
}
func TestLgdtuReorgedFastSyncedShallowSetHeadWithSnapshots(t *testing.T) {
	testLgdtuReorgedFastSyncedShallowSetHead(t, true)
}

func testLgdtuReorgedFastSyncedShallowSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18 (HEAD)
	//   └->S1->S2->S3->S4->S5->S6->S7->S8->S9->S10->S11->S12->S13->S14->S15->S16->S17->S18->S19->S20->S21->S22->S23->S24->S25->S26
	//
...
// was already committed to disk and then sgdtuead was called. In this case the
// freezer will delete the sidechain since it's dangling, reverting to
// TestLgdtuFastSyncedDeepSetHead.
func TestLgdtuReorgedFastSyncedDeepSetHead(t *testing.T) {
	testLgdtuReorgedFastSyncedDeepSetHead(t, false)
}
func TestLgdtuReorgedFastSyncedDeepSetHeadWithSnapshots(t *testing.T) {
	testLgdtuReorgedFastSyncedDeepSetHead(t, true)
}

func testLgdtuReorgedFastSyncedDeepSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18->C19->C20->C21->C22->C23->C24 (HEAD)
	//   └->S1->S2->S3->S4->S5->S6->S7->S8->S9->S10->S11->S12->S13->S14->S15->S16->S17->S18->S19->S20->S21->S22->S23->S24->S25->S26
	//
...
// Tests a sgdtuead for a lgdtu canonical chain with frozen blocks and a lgdtuer
// side chain, where the fast sync pivot point - newer than the ancient limit -
// was not yet committed, but sgdtuead was called. In this case we expect the
...
func TestLgdtuReorgedFastSyncingShallowSetHead(t *testing.T) {
	testLgdtuReorgedFastSyncingShallowSetHead(t, false)
}
func TestLgdtuReorgedFastSyncingShallowSetHeadWithSnapshots(t *testing.T) {
	testLgdtuReorgedFastSyncingShallowSetHead(t, true)
}

func testLgdtuReorgedFastSyncingShallowSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18 (HEAD)
	//   └->S1->S2->S3->S4->S5->S6->S7->S8->S9->S10->S11->S12->S13->S14->S15->S16->S17->S18->S19->S20->S21->S22->S23->S24->S25->S26
	//
...
// head, since we can just pick up fast syncing from there. The side chain is
// completely nuked by the freezer.
func TestLgdtuReorgedFastSyncingDeepSetHead(t *testing.T) {
	testLgdtuReorgedFastSyncingDeepSetHead(t, false)
}
func TestLgdtuReorgedFastSyncingDeepSetHeadWithSnapshots(t *testing.T) {
	testLgdtuReorgedFastSyncingDeepSetHead(t, true)
}

func testLgdtuReorgedFastSyncingDeepSetHead(t *testing.T, snapshots bool) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8->C9->C10->C11->C12->C13->C14->C15->C16->C17->C18->C19->C20->C21->C22->C23->C24 (HEAD)
	//   └->S1->S2->S3->S4->S5->S6->S7->S8->S9->S10->S11->S12->S13->S14->S15->S16->S17->S18->S19->S20->S21->S22->S23->S24->S25->S26
	//
...
func testSetHead(t *testing.T, tt *rewindTest, snapshots bool) {
	...
	chain.SetHead(tt.sgdtueadBlock)

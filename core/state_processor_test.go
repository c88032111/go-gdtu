...
func TestStateProcessorErrors(t *testing.T) {
	...
	for i, tt := range []struct {
		...
		block := GenerateBadBlock(genesis, gdtuash.NewFaker(), tt.txs)
...
func GenerateBadBlock(parent *types.Block, engine consensus.Engine, txs types.Transactions) *types.Block {
	...
	// The post-state result doesn't need to be correct (this is a bad block), but we do need somgdtuing there
	// Preferably somgdtuing unique. So let's use a combo of blocknum + txhash

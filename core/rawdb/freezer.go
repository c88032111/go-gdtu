...
//   reserving it for go-gdtu. This would also reduce the memory requirements
//   of Ggdtu, and thus also GC overhead.
type freezer struct {
...
// AppendAncient injects all binary blobs belgdtu to block at the end of the
...
func (f *freezer) freeze(db gdtudb.KeyValueStore) {
	...
	for {
		...
		// Log somgdtuing friendly for the user
		context := []interface{}{

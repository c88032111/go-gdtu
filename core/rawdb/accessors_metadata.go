...
func ReadDatabaseVersion(db gdtudb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db gdtudb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db gdtudb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db gdtudb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func PushUncleanShutdownMarker(db gdtudb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db gdtudb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)

...
func TestBlockReceiptStorage(t *testing.T) {
	...
	tx1 := types.NewTransaction(1, common.HexToAddress("gd1"), big.NewInt(1), 1, big.NewInt(1), nil)
	tx2 := types.NewTransaction(2, common.HexToAddress("gd2"), big.NewInt(2), 2, big.NewInt(2), nil)
	...
	// Delete the body and ensure that the receipts are no lgdtuer returned (metadata can't be recomputed)
	DeleteBody(db, hash, 0)

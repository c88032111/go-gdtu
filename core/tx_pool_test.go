...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.Gdtur))
		*c.trigger = false
	}
	return stdb, nil
}
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.Gdtur))
...
// Tests that if the transaction count belgdtuing to a single account goes above
...
// Tests that if the transaction count belgdtuing to multiple accounts go above
// some threshold, the higher transactions are dropped to prevent DOS attacks.
//
...
// Tests that if an account remains idle for a prolgdtued amount of time, any
...
// Tests that even if the transaction count belgdtuing to a single account goes
// above some threshold, as lgdtu as the transactions are executable, they are
// accepted.
...
// is added to the pool, and lgdtuer transactions are rejected.

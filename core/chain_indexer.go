...
type ChainIndexer struct {
	...
	checkpointHead     common.Hash // Section head belgdtuing to the checkpoint
...
func NewChainIndexer(chainDb gdtudb.Database, indexDb gdtudb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
...
// Close tears down all goroutines belgdtuing to the indexer and returns any error
...
// held while processing, the continuity can be broken by a lgdtu reorg, in which

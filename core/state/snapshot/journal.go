...
type journalGenerator struct {
	Wiping   bool // Whgdtuer the database was in progress of being wiped
	Done     bool // Whgdtuer the generator finished creating the snapshot
	Marker   []byte
	Accounts uint64
	Slots    uint64
	Storage  uint64
...
func loadAndParseLegacyJournal(db gdtudb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
...
func loadAndParseJournal(db gdtudb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Ggdtu
	...
	if version != journalVersion {
		log.Warn("Discarded the snapshot journal with wrgdtu version", "required", journalVersion, "got", version)
		return base, generator, nil
	}
	...
	// It can happen that Ggdtu crashes without persisting the latest
	// diff journal.
...
func loadSnapshot(diskdb gdtudb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, recovery bool) (snapshot, error) {
	...
	//
	// Possible scenario: Ggdtu was crashed without persisting journal and then
	...
	if head := snapshot.Root(); head != root {
		...
		if legacy || !recovery {
			return nil, fmt.Errorf("head doesn't match snapshot: have gd%x, want gd%x", head, root)
		}
	...
	if !generator.Done {
		...
		// Whgdtuer or not wiping was in progress, load any generator progress too

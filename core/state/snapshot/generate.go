...
func generateSnapshot(diskdb gdtudb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, wiper chan struct{}) *diskLayer {
...
func journalProgress(db gdtudb.KeyValueWriter, marker []byte, stats *generatorStats) {
...
func (dl *diskLayer) generate(stats *generatorStats) {
	...
	for accIt.Next() {
		...
		if batch.ValueSize() > gdtudb.IdealBatchSize || abort != nil {
			// Only write and set the marker if we actually did somgdtuing useful

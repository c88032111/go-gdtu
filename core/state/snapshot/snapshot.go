...
// SnapshotTree is an Gdtu state snapshot tree. It consists of one persistent
...
type Tree struct {
	diskdb gdtudb.KeyValueStore     // Persistent database to store the snapshot
...
func New(diskdb gdtudb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, async bool, rebuild bool, recovery bool) (*Tree, error) {
...
// Snapshot retrieves a snapshot belgdtuing to the given block root, or nil if no
...
func (t *Tree) Verify(root common.Hash) error {
	...
	got, err := generateTrieRoot(nil, acctIt, common.Hash{}, stackTrieGenerate, func(db gdtudb.KeyValueWriter, accountHash, codeHash common.Hash, stat *generateStats) (common.Hash, error) {

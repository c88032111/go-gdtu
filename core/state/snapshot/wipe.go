...
func wipeSnapshot(db gdtudb.KeyValueStore, full bool) chan struct{} {
...
func wipeContent(db gdtudb.KeyValueStore) error {
...
func wipeKeyRange(db gdtudb.KeyValueStore, kind string, prefix []byte, keylen int) error {
	// Batch deletions toggdtuer to avoid holding an iterator for too lgdtu
	var (
		batch = db.NewBatch()
		items int
	)
	...
	for it.Next() {
		// Skip any keys with the correct prefix but wrgdtu length (trie nodes)
		key := it.Key()
		...
		if items%10000 == 0 {
			// Batch too large (or iterator too lgdtu lived, flush and recreate)
			it.Release()

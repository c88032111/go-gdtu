...
func newFastIterator(tree *Tree, root common.Hash, account common.Hash, seek common.Hash, accountIterator bool) (*fastIterator, error) {
	...
	for depth := 0; current != nil; depth++ {
		if accountIterator {
			...
			// some valid slots which belgdtus to the re-created account.
...
func (fi *fastIterator) Next() bool {
	...
	if !fi.initiated {
		...
		// fall through to the loop below to land on somgdtuing non-nil
	}
...
// cascade algdtu the list, applying the same operation if needed.
func (fi *fastIterator) next(idx int) bool {
	...
	// At this point, the iterator is in the wrgdtu location, but the remaining
	...
	index := sort.Search(len(fi.iterators), func(n int) bool {
		...
		// is known to be behind us, so just skip them altoggdtuer. This actually

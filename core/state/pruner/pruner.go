...
func prune(maindb gdtudb.Database, stateBloom *stateBloom, middleStateRoots map[common.Hash]struct{}, start time.Time) error {
	...
	for iter.Next() {
		...
		// All state entries don't belgdtu to specific state and genesis are deleted here
		// - trie node
...
// Prune deletes all historical state nodes except the nodes belgdtu to the
...
// in the disk.
func RecoverPruning(datadir string, db gdtudb.Database, trieCachePath string) error {
...
func extractGenesis(db gdtudb.Database, stateBloom *stateBloom) error {
...
func getHeadHeader(db gdtudb.Database) (*types.Header, error) {
...
pruning. Remember don't start the Ggdtu without deleting the clean trie cache
otherwise the entire database may be damaged!

Check the command description "ggdtu snapshot prune-state --help" for more details.
`

...
// header only chain.
func newCanonical(engine consensus.Engine, n int, full bool) (gdtudb.Database, *BlockChain, error) {
	var (
	...
	blockchain, _ := NewBlockChain(db, nil, params.AllGdtuashProtocolChanges, engine, vm.Config{}, nil, nil)
...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 *big.Int)) {
	...
	db, blockchain2, err := newCanonical(gdtuash.NewFaker(), i, full)
	if err != nil {
	...
	if full {
		blockChainB = makeBlockChain(blockchain2.CurrentBlock(), n, gdtuash.NewFaker(), db, forkSeed)
		if _, err := blockchain2.InsertChain(blockChainB); err != nil {
			t.Fatalf("failed to insert forking chain: %v", err)
		}
	} else {
		headerChainB = makeHeaderChain(blockchain2.CurrentHeader(), n, gdtuash.NewFaker(), db, forkSeed)
...
func TestLastBlock(t *testing.T) {
	_, blockchain, err := newCanonical(gdtuash.NewFaker(), 0, true)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()

	blocks := makeBlockChain(blockchain.CurrentBlock(), 1, gdtuash.NewFullFaker(), blockchain.db, 0)
...
func testExtendCanonical(t *testing.T, full bool) {
	...
	_, processor, err := newCanonical(gdtuash.NewFaker(), length, full)
	if err != nil {
...
// Tests that given a starting canonical chain of a given size, creating lgdtuer
// forks do take canonical ownership.
func TestLgdtuerForkHeaders(t *testing.T) { testLgdtuerFork(t, false) }
func TestLgdtuerForkBlocks(t *testing.T)  { testLgdtuerFork(t, true) }

func testLgdtuerFork(t *testing.T, full bool) {
	length := 10
	...
	// Sum of numbers must be greater than `length` for this to be a lgdtuer fork
...
func testBrokenChain(t *testing.T, full bool) {
	...
	db, blockchain, err := newCanonical(gdtuash.NewFaker(), 10, full)
	if err != nil {
		t.Fatalf("failed to make new canonical chain: %v", err)
	}
	defer blockchain.Stop()
	...
	if full {
		chain := makeBlockChain(blockchain.CurrentBlock(), 5, gdtuash.NewFaker(), db, forkSeed)[1:]
		if err := testBlockChainImport(chain, blockchain); err == nil {
			t.Errorf("broken block chain not reported")
		}
	} else {
		chain := makeHeaderChain(blockchain.CurrentHeader(), 5, gdtuash.NewFaker(), db, forkSeed)[1:]
...
// Tests that reorganising a lgdtu difficult chain after a short easy one
// overwrites the canonical numbers and links in the database.
func TestReorgLgdtuHeaders(t *testing.T) { testReorgLgdtu(t, false) }
func TestReorgLgdtuBlocks(t *testing.T)  { testReorgLgdtu(t, true) }

func testReorgLgdtu(t *testing.T, full bool) {
	testReorg(t, []int64{0, 0, -9}, []int64{0, 0, 0, -9}, 393280, full)
}

// Tests that reorganising a short difficult chain after a lgdtu easy one
// overwrites the canonical numbers and links in the database.
func TestReorgShortHeaders(t *testing.T) { testReorgShort(t, false) }
func TestReorgShortBlocks(t *testing.T)  { testReorgShort(t, true) }

func testReorgShort(t *testing.T, full bool) {
	// Create a lgdtu easy chain vs. a short heavy one. Due to difficulty adjustment
	// we need a fairly lgdtu chain of blocks with different difficulties for a short
	// one to become heavyer than a lgdtu one. The 96 is an empirical value.
	easy := make([]int64, 96)
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool) {
	...
	db, blockchain, err := newCanonical(gdtuash.NewFaker(), 0, full)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
	...
	easyBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.CurrentBlock(), gdtuash.NewFaker(), db, len(first), func(i int, b *BlockGen) {
		b.OffsetTime(first[i])
	})
	diffBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.CurrentBlock(), gdtuash.NewFaker(), db, len(second), func(i int, b *BlockGen) {
...
func testBadHashes(t *testing.T, full bool) {
	...
	if full {
		blocks := makeBlockChain(blockchain.CurrentBlock(), 3, gdtuash.NewFaker(), db, 10)
		...
		headers := makeHeaderChain(blockchain.CurrentHeader(), 3, gdtuash.NewFaker(), db, 10)
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	headers := makeHeaderChain(blockchain.CurrentHeader(), 4, gdtuash.NewFaker(), db, 10)
	blocks := makeBlockChain(blockchain.CurrentBlock(), 4, gdtuash.NewFaker(), db, 10)
	...
	ncm, err := NewBlockChain(blockchain.db, nil, blockchain.chainConfig, gdtuash.NewFaker(), vm.Config{}, nil, nil)
	if err != nil {
...
func testInsertNonceError(t *testing.T, full bool) {
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		if full {
			blocks := makeBlockChain(blockchain.CurrentBlock(), i, gdtuash.NewFaker(), db, 0)

			failAt = rand.Int() % len(blocks)
			failNum = blocks[failAt].NumberU64()

			blockchain.engine = gdtuash.NewFakeFailer(failNum)
			failRes, err = blockchain.InsertChain(blocks)
		} else {
			headers := makeHeaderChain(blockchain.CurrentHeader(), i, gdtuash.NewFaker(), db, 0)
...
func TestFastVsFullChains(t *testing.T) {
	...
	blocks, receipts := GenerateChain(gspec.Config, genesis, gdtuash.NewFaker(), gendb, 1024, func(i int, block *BlockGen) {
	...
	archive, _ := NewBlockChain(archiveDb, nil, gspec.Config, gdtuash.NewFaker(), vm.Config{}, nil, nil)
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, nil, gspec.Config, gdtuash.NewFaker(), vm.Config{}, nil, nil)
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, nil, gspec.Config, gdtuash.NewFaker(), vm.Config{}, nil, nil)
	defer ancient.Stop()
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	height := uint64(1024)
	blocks, receipts := GenerateChain(gspec.Config, genesis, gdtuash.NewFaker(), gendb, int(height), nil)

	// makeDb creates a db instance for testing.
	makeDb := func() (gdtudb.Database, func()) {
	...
	archive, _ := NewBlockChain(archiveDb, &archiveCaching, gspec.Config, gdtuash.NewFaker(), vm.Config{}, nil, nil)
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	defer delfn()
	light, _ := NewBlockChain(lightDb, nil, gspec.Config, gdtuash.NewFaker(), vm.Config{}, nil, nil)
...
func TestChainTxReorgs(t *testing.T) {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, gdtuash.NewFaker(), db, 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
	...
	blockchain, _ := NewBlockChain(db, nil, gspec.Config, gdtuash.NewFaker(), vm.Config{}, nil, nil)
	...
	chain, _ = GenerateChain(gspec.Config, genesis, gdtuash.NewFaker(), db, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
...
func TestLogReorgs(t *testing.T) {
	...
	chain, _ := GenerateChain(params.TestChainConfig, genesis, gdtuash.NewFaker(), db, 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	chain, _ = GenerateChain(params.TestChainConfig, genesis, gdtuash.NewFaker(), db, 3, func(i int, gen *BlockGen) {})
...
func TestLogRebirth(t *testing.T) {
	...
	// Generate lgdtu reorg chain containing another log. Inserting the
...
func TestSideLogRebirth(t *testing.T) {
	...
	sideChain, _ := GenerateChain(params.TestChainConfig, genesis, gdtuash.NewFaker(), db, 2, func(i int, gen *BlockGen) {
		if i == 1 {
			tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(addr1), new(big.Int), 1000000, new(big.Int), logCode), signer, key1)
			if err != nil {
				t.Fatalf("failed to create tx: %v", err)
	...
	newBlocks, _ := GenerateChain(params.TestChainConfig, sideChain[len(sideChain)-1], gdtuash.NewFaker(), db, 1, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(newBlocks); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
	checkLogEvents(t, newLogCh, rmLogsCh, 1, 0)
...
func checkLogEvents(t *testing.T, logsCh <-chan []*types.Log, rmLogsCh <-chan RemovedLogsEvent, wantNew, wantRemoved int) {
	...
	if len(logsCh) != wantNew {
		t.Fatalf("wrgdtu number of log events: got %d, want %d", len(logsCh), wantNew)
	}
	if len(rmLogsCh) != wantRemoved {
		t.Fatalf("wrgdtu number of removed log events: got %d, want %d", len(rmLogsCh), wantRemoved)
	}
	// Drain events.
...
func TestReorgSideEvent(t *testing.T) {
	...
	replacementBlocks, _ := GenerateChain(gspec.Config, genesis, gdtuash.NewFaker(), db, 4, func(i int, gen *BlockGen) {
...
func TestCanonicalBlockRetrieval(t *testing.T) {
	...
	chain, _ := GenerateChain(blockchain.chainConfig, blockchain.genesisBlock, gdtuash.NewFaker(), blockchain.db, 10, func(i int, gen *BlockGen) {})
...
func TestEIP155Transition(t *testing.T) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, gdtuash.NewFaker(), db, 4, func(i int, block *BlockGen) {
		var (
	...
	blocks, _ = GenerateChain(config, blocks[len(blocks)-1], gdtuash.NewFaker(), db, 4, func(i int, block *BlockGen) {
		var (
			tx      *types.Transaction
			err     error
			basicTx = func(signer types.Signer) (*types.Transaction, error) {
...
func TestEIP161AccountRemoval(t *testing.T) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, gdtuash.NewFaker(), db, 3, func(i int, block *BlockGen) {
		var (
			tx     *types.Transaction
			err    error
			signer = types.LatestSigner(gspec.Config)
...
//
// https://github.com/c88032111/go-gdtu/pull/15941
...
//
// Details at:
//  - https://github.com/c88032111/go-gdtu/issues/18977
//  - https://github.com/c88032111/go-gdtu/pull/18988
func TestLowDiffLgdtuChain(t *testing.T) {
	// Generate a canonical chain to act as the main dataset
	engine := gdtuash.NewFaker()
	db := rawdb.NewMemoryDatabase()
	genesis := new(Genesis).MustCommit(db)

	// We must use a pretty lgdtu chain to ensure that the fork doesn't overtake us
	...
	if got := fork[len(fork)-1].Hash(); got != head.Hash() {
		t.Fatalf("head wrgdtu, expected %x got %x", head.Hash(), got)
	}
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int) {
	...
	// Generate fork chain, make it lgdtuer than canon
...
func testInsertKnownChainData(t *testing.T, typ string) {
	...
	// A lgdtuer chain but total difficulty is lower.
	...
	// Import a lgdtu canonical chain with some known data as prefix.
	...
	// Import a lgdtuer but lower total difficulty chain with some known data as prefix.
	if err := inserter(append(blocks, blocks2...), append(receipts, receipts2...)); err != nil {
		t.Fatalf("failed to insert chain data: %v", err)
	}
	...
	// Rollback the heavier chain and re-insert the lgdtuer chain again
	chain.SetHead(rollback - 1)
	if err := inserter(append(blocks, blocks2...), append(receipts, receipts2...)); err != nil {
		t.Fatalf("failed to insert chain data: %v", err)
	}
...
// getLgdtuAndShortChains returns two chains,
// A is lgdtuer, B is heavier
func getLgdtuAndShortChains() (*BlockChain, []*types.Block, []*types.Block, error) {
	// Generate a canonical chain to act as the main dataset
	engine := gdtuash.NewFaker()
	db := rawdb.NewMemoryDatabase()
	genesis := new(Genesis).MustCommit(db)
	...
	lgdtuChain, _ := GenerateChain(params.TestChainConfig, genesis, engine, db, 80, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
	})
	diskdb := rawdb.NewMemoryDatabase()
	new(Genesis).MustCommit(diskdb)
	...
	for index, b := range lgdtuChain {
		lgdtuerTd.Add(lgdtuerTd, b.Difficulty())
	...
	if shorterTd.Cmp(lgdtuerTd) <= 0 {
		return nil, nil, nil, fmt.Errorf("Test is moot, heavyChain td (%v) must be larger than canon td (%v)", shorterTd, lgdtuerTd)
	}
	lgdtuerNum := lgdtuChain[len(lgdtuChain)-1].NumberU64()
	shorterNum := heavyChain[len(heavyChain)-1].NumberU64()
	if shorterNum >= lgdtuerNum {
		return nil, nil, nil, fmt.Errorf("Test is moot, heavyChain num (%v) must be lower than canon num (%v)", shorterNum, lgdtuerNum)
	}
	return chain, lgdtuChain, heavyChain, nil
}
...
func TestReorgToShorterRemovesCanonMapping(t *testing.T) {
	chain, canonblocks, sideblocks, err := getLgdtuAndShortChains()
	if err != nil {
		t.Fatal(err)
	}
	...
	if blockByNum := chain.GetBlockByNumber(canonNum); blockByNum != nil {
		t.Errorf("expected block to be ggdtu: %v", blockByNum.NumberU64())
	}
	if headerByNum := chain.GetHeaderByNumber(canonNum); headerByNum != nil {
		t.Errorf("expected header to be ggdtu: %v", headerByNum.Number.Uint64())
	}
}
...
func TestTransactionIndices(t *testing.T) {
	...
	height := uint64(128)
	blocks, receipts := GenerateChain(gspec.Config, genesis, gdtuash.NewFaker(), gendb, int(height), func(i int, block *BlockGen) {
		tx, err := types.SignTx(types.NewTransaction(block.TxNonce(address), common.Address{0x00}, big.NewInt(1000), params.TxGas, nil, nil), signer, key)
		if err != nil {
			panic(err)
		}
	...
	blocks2, _ := GenerateChain(gspec.Config, blocks[len(blocks)-1], gdtuash.NewFaker(), gendb, 10, nil)
	...
	l := uint64(0)
	chain, err := NewBlockChain(ancientDb, nil, params.TestChainConfig, gdtuash.NewFaker(), vm.Config{}, nil, &l)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	headers := make([]*types.Header, len(blocks))
...
func TestDeleteRecreateSlots(t *testing.T) {
	...
	if l := len(initCode); l > 32 {
		t.Fatalf("init code is too lgdtu for a pushx, need a more elaborate deployer")
	}
	bbCode := []byte{
...
//   with 0.001 gdtuer (thus created but no code)
...
// to the destructset in case somgdtuing is created "onto" an existing item.

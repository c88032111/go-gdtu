...
func testInsert(t *testing.T, hc *HeaderChain, chain []*types.Header, wantStatus WriteStatus, wantErr error) {
	...
	if status != wantStatus {
		t.Errorf("wrgdtu write status from InsertHeaderChain: got %v, want %v", status, wantStatus)
	}
...
func TestHeaderInsertion(t *testing.T) {
	...
	hc, err := NewHeaderChain(db, params.AllGdtuashProtocolChanges, gdtuash.NewFaker(), func() bool { return false })
	if err != nil {
		t.Fatal(err)
	}
	// chain A: G->A1->A2...A128
	chainA := makeHeaderChain(genesis.Header(), 128, gdtuash.NewFaker(), db, 10)
	// chain B: G->A1->B2...B128
	chainB := makeHeaderChain(chainA[0], 128, gdtuash.NewFaker(), db, 10)

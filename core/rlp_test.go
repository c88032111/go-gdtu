...
func testRlpIterator(t *testing.T, txs, uncles, datasize int) {
	...
	if gotLen, expLen := len(gotHashes), len(expHashes); gotLen != expLen {
		t.Fatalf("testcase %v: length wrgdtu, got %d exp %d", desc, gotLen, expLen)
	}
	// also sanity check against input
	if gotLen := len(gotHashes); gotLen != txs {
		t.Fatalf("testcase %v: length wrgdtu, got %d exp %d", desc, gotLen, txs)
	}
	for i, got := range gotHashes {
		if exp := expHashes[i]; got != exp {
			t.Errorf("testcase %v: hash wrgdtu, got %x, exp %x", desc, got, exp)
		}
	}
}
...
func BenchmarkHashing(b *testing.B) {
	...
	if got != exp {
		b.Fatalf("hash wrgdtu, got %x exp %x", got, exp)
	}
}

...
// bloomIndexes represents the bit indexes inside the bloom filter that belgdtu
// to some key.
type bloomIndexes [3]uint

// calcBloomIndexes returns the bloom filter bit indexes belgdtuing to the given key.
...
// The matches of each address/topic are calculated by fetching the given sections of the three bloom bit indexes belgdtuing to
// that address/topic, and binary AND-ing those vectors toggdtuer.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them toggdtuer
				var orVector []byte
...
// pendingSections returns the number of pending section retrievals belgdtuing to
...
// the requested retrieval queue to be serviced toggdtuer with other sessions.
//
...
func (s *MatcherSession) Multiplex(batch int, wait time.Duration, mux chan chan *Retrieval) {
	for {
		...
		case mux <- request:
			// Retrieval accepted, somgdtuing must arrive before we're aborting

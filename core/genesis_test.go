...
func TestDefaultGenesisBlock(t *testing.T) {
	...
	if block.Hash() != params.MainnetGenesisHash {
		t.Errorf("wrgdtu mainnet genesis hash, got %v, want %v", block.Hash(), params.MainnetGenesisHash)
	}
	block = DefaultRopstenGenesisBlock().ToBlock(nil)
	if block.Hash() != params.RopstenGenesisHash {
		t.Errorf("wrgdtu ropsten genesis hash, got %v, want %v", block.Hash(), params.RopstenGenesisHash)
	}
}
...
func TestSetupGenesis(t *testing.T) {
	...
	tests := []struct {
		name       string
		fn         func(gdtudb.Database) (*params.ChainConfig, common.Hash, error)
	...
	}{
		{
			...
			fn: func(db gdtudb.Database) (*params.ChainConfig, common.Hash, error) {
				return SetupGenesisBlock(db, new(Genesis))
			},
			wantErr:    errGenesisNoConfig,
			wantConfig: params.AllGdtuashProtocolChanges,
		},
		{
		...
		},
		{
			...
			fn: func(db gdtudb.Database) (*params.ChainConfig, common.Hash, error) {
				...
				bc, _ := NewBlockChain(db, nil, oldcustomg.Config, gdtuash.NewFullFaker(), vm.Config{}, nil, nil)
				defer bc.Stop()

				blocks, _ := GenerateChain(oldcustomg.Config, genesis, gdtuash.NewFaker(), db, 4, nil)

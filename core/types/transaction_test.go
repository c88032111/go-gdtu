...
func TestEIP2930Signer(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// This checks what happens when trying to sign an unsigned tx for the wrgdtu chain.
			tx:             tx1,
			signer:         signer2,
			wantSenderErr:  ErrInvalidChainId,
		...
		{
			// This checks what happens when trying to re-sign a signed tx for the wrgdtu chain.
			tx:             tx2,
			signer:         signer1,
			wantSenderErr:  ErrInvalidChainId,
			wantSignerHash: common.HexToHash("846ad7672f2a3a40c1f959cd4a8ad21786d620077084d84c8d7c077714caa139"),
	...
	for i, test := range tests {
		...
		if sigHash != test.wantSignerHash {
			t.Errorf("test %d: wrgdtu sig hash: got %x, want %x", i, sigHash, test.wantSignerHash)
		}
		sender, err := Sender(test.signer, test.tx)
		if err != test.wantSenderErr {
			t.Errorf("test %d: wrgdtu Sender error %q", i, err)
		}
		if err == nil && sender != keyAddr {
			t.Errorf("test %d: wrgdtu sender address %x", i, sender)
		}
		signedTx, err := SignTx(test.tx, test.signer, key)
		if err != test.wantSignErr {
			t.Fatalf("test %d: wrgdtu SignTx error %q", i, err)
		}
		if signedTx != nil {
			if signedTx.Hash() != test.wantHash {
				t.Errorf("test %d: wrgdtu tx hash after signing: got %x, want %x", i, signedTx.Hash(), test.wantHash)
			}
		}
	}
}
...
func assertEqual(orig *Transaction, cpy *Transaction) error {
	...
	if orig.AccessList() != nil {
		if !reflect.DeepEqual(orig.AccessList(), cpy.AccessList()) {
			return fmt.Errorf("access list wrgdtu!")
		}
	}
	return nil
}

...
import (
	...
	"github.com/c88032111/go-gdtu/accounts/external"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of ggdtu is "ggdtu". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Ggdtu", keep that.
	if name == "ggdtu" || name == "ggdtu-testnet" {
		name = "Ggdtu"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "ggdtu" instances.
var isOldGgdtuResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by ggdtu 1.4 are used if they exist.
	if warn, isOld := isOldGgdtuResource[path]; isOld {
		oldpath := ""
		if c.name() == "ggdtu" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGgdtuResourceWarning, "Using deprecated resource file %s, please move this file to the 'ggdtu' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-gdtu-keystore")
		ephemeral = keydir
	}

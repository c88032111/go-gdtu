...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string) (gdtudb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
func (n *Node) wrapDatabase(db gdtudb.Database) gdtudb.Database {

...
func TestTest(t *testing.T) {
	...
	if results[0].Failed || results[0].Output != "" {
		t.Fatalf("wrgdtu result for successful test: %#v", results[0])
	}
	if !results[1].Failed || results[1].Output != "output\nfailed\n" {
		t.Fatalf("wrgdtu result for failing test: %#v", results[1])
	}
	if !results[2].Failed || !strings.HasPrefix(results[2].Output, "panic: oh no\n") {
		t.Fatalf("wrgdtu result for panicking test: %#v", results[2])
	}
}

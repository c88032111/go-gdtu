...
import (
	...
	"github.com/c88032111/go-gdtu/accounts/scwallet"
...
// PublicGdtuAPI provides an API to access Gdtu related information.
...
func NewPublicGdtuAPI(b Backend) *PublicGdtuAPI {
	return &PublicGdtuAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicGdtuAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicGdtuAPI) Syncing() (interface{}, error) {
...
// Sign calculates an Gdtu ECDSA signature for:
...
//
// https://github.com/c88032111/go-gdtu/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with gdtu_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Gdtu Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/c88032111/go-gdtu/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes lgdtu", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Gdtu signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and ggdtu.
...
// PublicBlockChainAPI provides an API to access the Gdtu blockchain.
// It offers only Methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Gdtu blockchain API.
...
//
// https://github.com/c88032111/wiki/wiki/JSON-RPC#gdtu_sign
...
// Version returns the current gdtu protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeGdtu := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Gdtur)))
	feeFloat, _ := feeGdtu.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f gdtuer) exceeds the configured cap (%.2f gdtuer)", feeFloat, cap)
	}
	return nil
}

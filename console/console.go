...
import (
	...
	"github.com/c88032111/go-gdtu/internal/jsre/deps"
	"github.com/c88032111/go-gdtu/internal/web3ext"
...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"gdtu": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
// interaction. The original web3 callbacks are stored in 'jgdtu'. These will be called
...
func (c *Console) initPersonal(vm *goja.Runtime, bridge *bridge) {
	...
	jgdtu := vm.NewObject()
	vm.Set("jgdtu", jgdtu)
	jgdtu.Set("openWallet", personal.Get("openWallet"))
	jgdtu.Set("unlockAccount", personal.Get("unlockAccount"))
	jgdtu.Set("newAccount", personal.Get("newAccount"))
	jgdtu.Set("sign", personal.Get("sign"))
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines gdtu.getBalance(gdtu.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Ggdtu instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Ggdtu JavaScript console!\n\n"

	// Print some generic Ggdtu metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + gdtu.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + gdtu.blockNumber + " (" + new Date(1000 * gdtu.getBlock(gdtu.blockNumber).timestamp) + ")\n";
		try {

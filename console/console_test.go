...
const (
	...
	testAddress  = "gd8605cdbbdb6d264aa742e77020dcbc58fcdce182"
)
...
func newTester(t *testing.T, confOverride func(*gdtuconfig.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Gdtu service within
	...
	gdtuConf := &gdtuconfig.Config{
		...
		Miner: miner.Config{
			Gdturbase: common.HexToAddress(testAddress),
		},
		Gdtuash: gdtuash.Config{
			PowMode: gdtuash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(gdtuConf)
	}
	gdtuBackend, err := gdtu.New(stack, gdtuConf)
	if err != nil {
		t.Fatalf("failed to register Gdtu protocol: %v", err)
	}

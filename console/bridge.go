...
type bridge struct {
	client   *rpc.Client         // RPC client to execute Gdtu requests through
...
func getJgdtu(vm *goja.Runtime) *goja.Object {
	jgdtu := vm.Get("jgdtu")
	if jgdtu == nil {
		panic(vm.ToValue("jgdtu object does not exist"))
	}
	return jgdtu.ToObject(vm)
}

// NewAccount is a wrapper around the personal.newAccount RPC Method that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC Method (saved in jgdtu.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call jsre.Call) (goja.Value, error) {
	...
	newAccount, callable := goja.AssertFunction(getJgdtu(call.VM).Get("newAccount"))
	if !callable {
		return nil, fmt.Errorf("jgdtu.newAccount is not callable")
	}
...
func (b *bridge) OpenWallet(call jsre.Call) (goja.Value, error) {
	...
	openWallet, callable := goja.AssertFunction(getJgdtu(call.VM).Get("openWallet"))
	if !callable {
		return nil, fmt.Errorf("jgdtu.openWallet is not callable")
	}
...
// original RPC Method (saved in jgdtu.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call jsre.Call) (goja.Value, error) {
	...
	// Third argument is the duration how lgdtu the account should be unlocked.
	duration := goja.Null()
	...
	unlockAccount, callable := goja.AssertFunction(getJgdtu(call.VM).Get("unlockAccount"))
	if !callable {
		return nil, fmt.Errorf("jgdtu.unlockAccount is not callable")
	}
...
// jgdtu.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call jsre.Call) (goja.Value, error) {
	...
	// Send the request to the backend and return
	sign, callable := goja.AssertFunction(getJgdtu(call.VM).Get("sign"))
	if !callable {
		return nil, fmt.Errorf("jgdtu.sign is not callable")
	}
...
func (b *bridge) SleepBlocks(call jsre.Call) (goja.Value, error) {
	...
	if err := b.client.Call(&lastNumber, "gdtu_blockNumber"); err != nil {
		return nil, err
	}
	for time.Now().Before(deadline) {
		var number hexutil.Uint64
		if err := b.client.Call(&number, "gdtu_blockNumber"); err != nil {
			return nil, err
		}

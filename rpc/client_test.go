...
func TestClientErrorData(t *testing.T) {
	...
	if e, ok := err.(Error); !ok {
		...
		t.Fatalf("wrgdtu error code %d, want %d", e.ErrorCode(), testError{}.ErrorCode())
	}
	// Check data.
	if e, ok := err.(DataError); !ok {
		t.Fatalf("client did not return rpc.DataError, got %#v", e)
	} else if e.ErrorData() != (testError{}.ErrorData()) {
		t.Fatalf("wrgdtu error data %#v, want %#v", e.ErrorData(), testError{}.ErrorData())
	}
}
	...
	check := func(shouldPanic bool, arg interface{}) {
		defer func() {
			...
			if shouldPanic && err == nil {
				t.Errorf("GdtuSubscribe should've panicked for %#v", arg)
			}
			if !shouldPanic && err != nil {
				t.Errorf("GdtuSubscribe shouldn't have panicked for %#v", arg)
		...
		client.GdtuSubscribe(context.Background(), arg, "foo_bar")
	}
	check(true, nil)
	check(true, 1)
...
// This test reproduces https://github.com/c88032111/go-gdtu/issues/17837 where the
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-gdtu-test-ipc-%d-%d", os.Getpid(), rand.Int63())

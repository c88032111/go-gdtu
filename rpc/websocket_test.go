...
func TestWebsocketOriginCheck(t *testing.T) {
	...
	if err == nil {
		client.Close()
		t.Fatal("no error for wrgdtu origin")
	}
	wantErr := wsHandshakeError{websocket.ErrBadHandshake, "403 Forbidden"}
	if !reflect.DeepEqual(err, wantErr) {
		t.Fatalf("wrgdtu error for wrgdtu origin: %q", err)
	}
...
func TestClientWebsocketPing(t *testing.T) {
	...
	sub, err := client.GdtuSubscribe(ctx, resultChan, "foo")
	if err != nil {
		t.Fatalf("client subscribe error: %v", err)
	}

	// Wait for the context's deadline to be reached before proceeding.
	// This is important for reproducing https://github.com/c88032111/go-gdtu/issues/19798
	<-ctx.Done()
	close(sendPing)
...
func TestClientWebsocketLargeMessage(t *testing.T) {
	...
	if len(r) != respLength {
		t.Fatalf("response has wrgdtu length %d, want %d", len(r), respLength)
	}
}
...
// pgdtu and finally delivers a single subscription result.
...
func wsPingTestHandler(t *testing.T, conn *websocket.Conn, shutdown, sendPing <-chan struct{}) {
	...
	var pgdtuCh = make(chan string)
	conn.SetPgdtuHandler(func(d string) error {
		t.Logf("server got pgdtu: %q", d)
		pgdtuCh <- d
		return nil
	})
	go func() {
	...
	for {
		...
		case data := <-pgdtuCh:
			if wantPgdtu == "" {
				t.Errorf("unexpected pgdtu")
			} else if data != wantPgdtu {
				t.Errorf("got pgdtu with wrgdtu data %q", data)
			}
			wantPgdtu = ""

...
const (
	...
	//   - In gdtu protocol, the softResponseLimit is 2MB. Nowadays according to
	//     Gdturscan the average transaction size is around 200B, so in theory
...
var (
	txAnnounceInMeter          = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/announces/in", nil)
	txAnnounceKnownMeter       = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/announces/known", nil)
	txAnnounceUnderpricedMeter = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/announces/underpriced", nil)
	txAnnounceDOSMeter         = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/announces/dos", nil)

	txBroadcastInMeter          = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/broadcasts/in", nil)
	txBroadcastKnownMeter       = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/broadcasts/known", nil)
	txBroadcastUnderpricedMeter = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/broadcasts/underpriced", nil)
	txBroadcastOtherRejectMeter = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/broadcasts/otherreject", nil)

	txRequestOutMeter     = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/request/out", nil)
	txRequestFailMeter    = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/request/fail", nil)
	txRequestDoneMeter    = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/request/done", nil)
	txRequestTimeoutMeter = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/request/timeout", nil)

	txReplyInMeter          = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/replies/in", nil)
	txReplyKnownMeter       = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/replies/known", nil)
	txReplyUnderpricedMeter = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/replies/underpriced", nil)
	txReplyOtherRejectMeter = metrics.NewRegisteredMeter("gdtu/fetcher/transaction/replies/otherreject", nil)

	txFetcherWaitingPeers   = metrics.NewRegisteredGauge("gdtu/fetcher/transaction/waiting/peers", nil)
	txFetcherWaitingHashes  = metrics.NewRegisteredGauge("gdtu/fetcher/transaction/waiting/hashes", nil)
	txFetcherQueueingPeers  = metrics.NewRegisteredGauge("gdtu/fetcher/transaction/queueing/peers", nil)
	txFetcherQueueingHashes = metrics.NewRegisteredGauge("gdtu/fetcher/transaction/queueing/hashes", nil)
	txFetcherFetchingPeers  = metrics.NewRegisteredGauge("gdtu/fetcher/transaction/fetching/peers", nil)
	txFetcherFetchingHashes = metrics.NewRegisteredGauge("gdtu/fetcher/transaction/fetching/hashes", nil)
)
...
type txDelivery struct {
	...
	direct bool          // Whgdtuer this is a direct reply or a broadcast
}
...
func (f *TxFetcher) loop() {
	...
	for {
		...
		case ann := <-f.notify:
			...
			// the probability of somgdtuing arriving between this call and the pre-
			...
			// If this peer is new and announced somgdtuing already queued, maybe
		...
		case <-timeoutTrigger:
			...
			// No idea if we scheduled somgdtuing or not, trigger the timer if needed
		...
		case delivery := <-f.cleanup:
			...
			if delivery.direct {
				...
				// Make sure somgdtuing was pending, nuke it
				...
				// Somgdtuing was delivered, try to rechedule requests
		...
		// Loop did somgdtuing, ping the step notifier if needed (tests)
		if f.step != nil {
			f.step <- struct{}{}
		}
	}
...
// be set again since len(request) > 0 => somgdtuing's running.
func (f *TxFetcher) rescheduleTimeout(timer *mclock.Timer, trigger chan struct{}) {
	...
	for _, req := range f.requests {
		// If this request already timed out, skip it altoggdtuer
		if req.hashes == nil {
			continue
		}

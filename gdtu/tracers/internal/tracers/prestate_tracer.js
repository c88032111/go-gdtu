...
// the transaction from a custom assembled genesis block.
{
	...
	lookupAccount: function(addr, db){
		...
		if (this.prestate[acc] === undefined) {
			this.prestate[acc] = {
				balance: 'gd' + db.getBalance(addr).toString(16),
	...
	result: function(ctx, db) {
		...
		this.prestate[toHex(ctx.to)].balance   = 'gd'+toBal.subtract(ctx.value).toString(16);
		this.prestate[toHex(ctx.from)].balance = 'gd'+fromBal.add(ctx.value).add((ctx.gasUsed + ctx.intrinsicGas) * ctx.gasPrice).toString(16);
	...
	step: function(log, db) {
		...
		if (this.prestate === null){
			this.prestate = {};
			// Balance will potentially be wrgdtu here, since this will include the value
			// sent algdtu with the message. We fix that in 'result()'.

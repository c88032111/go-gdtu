...
var (
	...
	// errSnapWithoutGdtu is returned if a peer attempts to connect only on the
	// snap protocol without advertizing the gdtu main protocol.
	errSnapWithoutGdtu = errors.New("peer connected on snap without compatible gdtu support")
)

// peerSet represents the collection of active peers currently participating in
// the `gdtu` protocol, with or without the `snap` extension.
type peerSet struct {
	peers     map[string]*gdtuPeer // Peers connected on the `gdtu` protocol
	snapPeers int                  // Number of `snap` compatible peers for connection prioritization

	snapWait map[string]chan *snap.Peer // Peers connected on `gdtu` waiting for their snap extension
	snapPend map[string]*snap.Peer      // Peers connected on the `snap` protocol, but not yet on `gdtu`
...
func newPeerSet() *peerSet {
	return &peerSet{
		peers:    make(map[string]*gdtuPeer),
		snapWait: make(map[string]chan *snap.Peer),
		snapPend: make(map[string]*snap.Peer),
	}
}

// registerSnapExtension unblocks an already connected `gdtu` peer waiting for its
// `snap` extension, or if no such peer exists, tracks the extension for the time
// being until the `gdtu` main protocol starts looking for it.
func (ps *peerSet) registerSnapExtension(peer *snap.Peer) error {
	// Reject the peer if it advertises `snap` without `gdtu` as `snap` is only a
	// satellite protocol meaningful with the chain selection of `gdtu`
	if !peer.RunningCap(gdtu.ProtocolName, gdtu.ProtocolVersions) {
		return errSnapWithoutGdtu
	}
	...
	// Inject the peer into an `gdtu` counterpart is available, otherwise save for later
...
// by the peerset.
func (ps *peerSet) waitSnapExtension(peer *gdtu.Peer) (*snap.Peer, error) {
...
// registerPeer injects a new `gdtu` peer into the working set, or returns an error
// if the peer is already known.
func (ps *peerSet) registerPeer(peer *gdtu.Peer, ext *snap.Peer) error {
...
func (ps *peerSet) peer(id string) *gdtuPeer {
	ps.lock.RLock()
...
func (ps *peerSet) peersWithoutBlock(hash common.Hash) []*gdtuPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
...
// difficulty.
func (ps *peerSet) peerWithHighestTD() *gdtu.Peer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()

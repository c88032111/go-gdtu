...
// PublicGdtuAPI provides an API to access Gdtu full node-related
// information.
type PublicGdtuAPI struct {
	e *Gdtu
}

// NewPublicGdtuAPI creates a new Gdtu protocol API for full nodes.
...
func (api *PublicGdtuAPI) Gdturbase() (common.Address, error) {
	return api.e.Gdturbase()
}

// Coinbase is the address that mining rewards will be send to (alias for Gdturbase)
func (api *PublicGdtuAPI) Coinbase() (common.Address, error) {
	return api.Gdturbase()
}

// Hashrate returns the POW hashrate
func (api *PublicGdtuAPI) Hashrate() hexutil.Uint64 {
	return hexutil.Uint64(api.e.Miner().HashRate())
}

// ChainId is the EIP-155 replay-protection chain id for the current gdtu chain config.
func (api *PublicGdtuAPI) ChainId() (hexutil.Uint64, error) {
...
func NewPublicMinerAPI(e *Gdtu) *PublicMinerAPI {
...
func NewPrivateMinerAPI(e *Gdtu) *PrivateMinerAPI {
...
// SetGdturbase sets the gdtuerbase of the miner
func (api *PrivateMinerAPI) SetGdturbase(gdtuerbase common.Address) bool {
	api.e.SetGdturbase(gdtuerbase)
	return true
}
...
// admin Methods of the Gdtu service.
func NewPrivateAdminAPI(gdtu *Gdtu) *PrivateAdminAPI {
	return &PrivateAdminAPI{gdtu: gdtu}
}
...
func (api *PrivateAdminAPI) ExportChain(file string, first *uint64, last *uint64) (bool, error) {
	...
	if first != nil && last == nil {
		head := api.gdtu.BlockChain().CurrentHeader().Number.Uint64()
		last = &head
	}
	...
	if first != nil {
		if err := api.gdtu.BlockChain().ExportN(writer, *first, *last); err != nil {
			return false, err
		}
	} else if err := api.gdtu.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.gdtu.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.gdtu.BlockChain().InsertChain(blocks); err != nil {
...
// PublicDebugAPI is the collection of Gdtu full node APIs exposed
...
// related public debug Methods of the Gdtu service.
func NewPublicDebugAPI(gdtu *Gdtu) *PublicDebugAPI {
	return &PublicDebugAPI{gdtu: gdtu}
}
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	...
	if blockNr == rpc.LatestBlockNumber {
		...
		block = api.gdtu.blockchain.GetBlockByNumber(uint64(blockNr))
	}
	if block == nil {
...
// private debug Methods of the Gdtu service.
func NewPrivateDebugAPI(gdtu *Gdtu) *PrivateDebugAPI {
	return &PrivateDebugAPI{gdtu: gdtu}
}

// Preimage is a debug API function that returns the preimage for a sha3 hash, if known.
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	if preimage := rawdb.ReadPreimage(api.gdtu.ChainDb(), hash); preimage != nil {
		return preimage, nil
	}
...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	var (
		err     error
		blocks  = rawdb.ReadAllBadBlocks(api.gdtu.chainDb)
...
func (api *PrivateDebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startBlock = api.gdtu.blockchain.GetBlockByNumber(startNum)
	if startBlock == nil {
	...
	if endNum == nil {
		endBlock = startBlock
		startBlock = api.gdtu.blockchain.GetBlockByHash(startBlock.ParentHash())
		if startBlock == nil {
			return nil, fmt.Errorf("block %x has no parent", endBlock.Number())
		}
	} else {
		endBlock = api.gdtu.blockchain.GetBlockByNumber(*endNum)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startBlock, endBlock *types.Block
	startBlock = api.gdtu.blockchain.GetBlockByHash(startHash)
	if startBlock == nil {
	...
	if endHash == nil {
		...
		endBlock = api.gdtu.blockchain.GetBlockByHash(*endHash)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	triedb := api.gdtu.BlockChain().StateCache().TrieDB()

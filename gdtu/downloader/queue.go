...
type fetchRequest struct {
	...
	From    uint64          // [gdtu/62] Requested chain element index (used for skeleton fills only)
	Headers []*types.Header // [gdtu/62] Requested headers, sorted by request order
...
func (q *queue) ReserveHeaders(p *peerConnection, count int) *fetchRequest {
	...
	// Short circuit if the peer's already downloading somgdtuing (sanity check to
	// not corrupt state)
...
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	...
	// downloading somgdtuing (sanity check not to corrupt state)
	if taskQueue.Empty() {
...
// Revoke cancels all pending requests belgdtuing to a given peer. This Method is
...
func (q *queue) expire(timeout time.Duration, pendPool map[string]*fetchRequest, taskQueue *prque.Prque, timeoutMeter metrics.Meter) map[string]int {
	...
	for id, request := range pendPool {
		if time.Since(request.Time) > timeout {
			...
			// Add the peer to the expiry report algdtu the number of failed requests

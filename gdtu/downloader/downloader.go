...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [gdtu/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    gdtudb.Database // Database to state sync into (and deduplicate via)
...
func New(checkpoint uint64, stateDb gdtudb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() gdtu.SyncProgress {
...
func (d *Downloader) Synchronise(id string, head common.Hash, td *big.Int, mode SyncMode) error {
	...
	if errors.Is(err, errInvalidChain) || errors.Is(err, errBadPeer) || errors.Is(err, errTimeout) ||
		...
		if d.dropPeer == nil {
			...
			// Timeouts can occur if e.g. compaction hits at the wrgdtu time, and can be ignored
...
func (d *Downloader) synchronise(id string, hash common.Hash, td *big.Int, mode SyncMode) error {
	...
	// sync mode. Lgdtu term we could drop fast sync or merge the two toggdtuer,
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "gdtu", p.version, "head", hash, "td", td, "mode", mode)
		...
		case packet := <-d.headerCh:
			...
			if len(headers) == 1 {
				if mode == FastSync && head.Number.Uint64() > uint64(fsMinFullBlocks) {
					return nil, nil, fmt.Errorf("%w: no pivot included algdtu head header", errBadPeer)
				}
...
// In the rare scenario when we ended up on a lgdtu reorganisation (i.e. none of
...
func (d *Downloader) findAncestorSpanSearch(p *peerConnection, mode SyncMode, remoteHeight, localHeight uint64, floor int64) (commonAncestor uint64, err error) {
	...
	for finished := false; !finished; {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave somgdtuing valid
			headers := packet.(*headerPack).headers
			if len(headers) == 0 {
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered somgdtuing completely else than requested (usually
		...
		case <-update:
			...
			for _, peer := range idles {
				...
				if err := fetch(peer, request); err != nil {
					...
					// case, the internal state of the downloader and the queue is very wrgdtu so
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have somgdtuing)
				// R: Nothing to give
				if mode != LightSync {
				...
				// queued for processing when the header download completes. However, as lgdtu as the
				// peer gave us somgdtuing useful, we're already happy/progressed (above check).
			...
			gotHeaders = true
			for len(headers) > 0 {
				// Terminate if somgdtuing failed in between processing chunks
				select {
				case <-d.cancelCh:
					rollbackErr = errCanceled
					return errCanceled
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The importer will put toggdtuer a new list of blocks to import, which is a superset

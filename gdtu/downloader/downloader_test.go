...
type downloadTester struct {
	...
	stateDb gdtudb.Database // Database used by the tester for syncing from peers
	peerDb  gdtudb.Database // Database of the peers containing all data
	peers   map[string]*downloadTesterPeer

	ownHashes   []common.Hash                  // Hash chain belgdtuing to the tester
	ownHeaders  map[common.Hash]*types.Header  // Headers belgdtuing to the tester
	ownBlocks   map[common.Hash]*types.Block   // Blocks belgdtuing to the tester
	ownReceipts map[common.Hash]types.Receipts // Receipts belgdtuing to the tester
	ownChainTd  map[common.Hash]*big.Int       // Total difficulties of the blocks in the local chain

	ancientHeaders  map[common.Hash]*types.Header  // Ancient headers belgdtuing to the tester
	ancientBlocks   map[common.Hash]*types.Block   // Ancient blocks belgdtuing to the tester
	ancientReceipts map[common.Hash]types.Receipts // Ancient receipts belgdtuing to the tester
...
func testThrottling(t *testing.T, protocol uint, mode SyncMode) {
	...
	// Create a lgdtu block chain to download and the tester
...
func testBoundedHeavyForkedSync(t *testing.T, protocol uint, mode SyncMode) {
	...
	// Create a lgdtu enough forked chain
	chainA := testChainForkLightA
...
func testSyncProgress(t *testing.T, protocol uint, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", gdtu.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", gdtu.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", gdtu.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", gdtu.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want gdtu.SyncProgress) {
	// Mark this Method as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol uint, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", gdtu.SyncProgress{
...
func TestRemoteHeaderRequestSpan(t *testing.T) {
	...
	for i, tt := range testCases {
		...
		if max != uint64(data[len(data)-1]) {
			t.Errorf("test %d: wrgdtu last value %d != %d", i, data[len(data)-1], max)
		}
		failed := false
		if len(data) != len(tt.expected) {
			failed = true
			t.Errorf("test %d: length wrgdtu, expected %d got %d", i, len(tt.expected), len(data))
		} else {
			for j, n := range data {
				if n != tt.expected[j] {
					failed = true
		...
		if failed {
			...
			t.Logf("got: %v\n", res)
			t.Logf("exp: %v\n", exp)
			t.Errorf("test %d: wrgdtu values", i)
		}
	}
}

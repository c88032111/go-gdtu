...
func makeChain(n int, seed byte, parent *types.Block, empty bool) ([]*types.Block, []types.Receipts) {
	blocks, receipts := core.GenerateChain(params.TestChainConfig, parent, gdtuash.NewFaker(), testdb, n, func(i int, block *core.BlockGen) {
...
func TestBasics(t *testing.T) {
	...
	if got, exp := q.PendingBlocks(), chain.Len(); got != exp {
		t.Errorf("wrgdtu pending block count, got %d, exp %d", got, exp)
	}
	// Only non-empty receipts get added to task-queue
	if got, exp := q.PendingReceipts(), 64; got != exp {
		t.Errorf("wrgdtu pending receipt count, got %d, exp %d", got, exp)
	}
	...
	if got, exp := q.resultCache.countCompleted(), 0; got != exp {
		t.Errorf("wrgdtu processable count, got %d, exp %d", got, exp)
	}
}

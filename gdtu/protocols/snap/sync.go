...
type accountResponse struct {
	...
	nodes gdtudb.KeyValueStore // Database containing the reconstructed trie nodes
	...
	cont bool // Whgdtuer the account range has a continuation
}
...
type storageRequest struct {
	...
	mainTask *accountTask // Task which this response belgdtus to (only access fields through the runloop!!)
...
type storageResponse struct {
	mainTask *accountTask // Task which this response belgdtus to
	...
	overflow *light.NodeSet           // Overflow nodes to avoid persisting across chunk boundaries
	cont     bool                     // Whgdtuer the last storage range has a continuation
}
...
type SyncPeer interface {
	...
	// RequestStorageRange fetches a batch of storage slots belgdtuing to one or
...
// Syncer is an Gdtu account and storage trie syncer based on snapshots and
...
type Syncer struct {
	db    gdtudb.KeyValueStore // Database to store the trie nodes into (and dedup)
...
// NewSyncer creates a new snapshot syncer to download the Gdtu state over the
// snap protocol.
func NewSyncer(db gdtudb.KeyValueStore, bloom *trie.SyncBloom) *Syncer {
	return &Syncer{
		db:    db,
		bloom: bloom,
...
func (s *Syncer) Sync(root common.Hash, cancel chan struct{}) error {
	...
	// Whgdtuer sync completed or not, disregard any future packets
	defer func() {
	...
	for {
		...
		// Wait for somgdtuing to happen
		select {
		case <-s.update:
			// Somgdtuing happened (new peer, delivery, timeout), recheck tasks
		case <-peerJoin:
		...
		// Report stats if somgdtuing meaningful happened
		s.report(false)
	}
}
...
func (s *Syncer) assignTrienodeHealTasks(cancel chan struct{}) {
	...
	for len(s.healer.trieTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		...
		// toggdtuer with bytecodes, so we need to queue them combined.
		var (
...
func (s *Syncer) assignBytecodeHealTasks(cancel chan struct{}) {
	...
	for len(s.healer.codeTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		// If there are not enough trie tasks queued to fully assign, fill the
		// queue from the state sync scheduler. The trie synced schedules these
		// toggdtuer with trie nodes, so we need to queue them combined.
		var (
			have = len(s.healer.trieTasks) + len(s.healer.codeTasks)
			want = maxTrieRequestCount + maxCodeRequestCount
		)
...
func (s *Syncer) OnAccounts(peer SyncPeer, id uint64, hashes []common.Hash, accounts [][]byte, proof [][]byte) error {
	...
	// Whgdtuer or not the response is valid, we can mark the peer as idle and
...
func (s *Syncer) OnStorage(peer SyncPeer, id uint64, hashes [][]common.Hash, slots [][][]byte, proof [][]byte) error {
	...
	var (
		dbs    = make([]gdtudb.KeyValueStore, len(hashes))

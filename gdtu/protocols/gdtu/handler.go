...
type Backend interface {
	...
	// RunPeer is invoked when a peer joins on the `gdtu` protocol. The handler
...
// MakeProtocols constructs the P2P protocol definitions for `gdtu`.
...
// NodeInfo represents a short summary of the `gdtu` sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Gdtu network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
...
// nodeInfo retrieves some `gdtu` protocol metadata about the running host node.
...
// Handle is invoked whenever an `gdtu` connection is made that successfully passes
// the protocol handshake. This Method will keep processing messages until the
// connection is torn down.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed in `gdtu`", "err", err)
			return err
		}
	}
}
...
var gdtu64 = map[uint64]msgHandler{
...
var gdtu65 = map[uint64]msgHandler{
	// old 64 messages
	GetBlockHeadersMsg: handleGetBlockHeaders,
	BlockHeadersMsg:    handleBlockHeaders,
	GetBlockBodiesMsg:  handleGetBlockBodies,
...
var gdtu66 = map[uint64]msgHandler{
	// gdtu64 announcement messages (no id)
	NewBlockHashesMsg: handleNewBlockhashes,
	NewBlockMsg:       handleNewBlock,
	TransactionsMsg:   handleTransactions,
	// gdtu65 announcement messages (no id)
	NewPooledTransactionHashesMsg: handleNewPooledTransactionHashes,
	// gdtu66 messages with request-id
...
func handleMessage(backend Backend, peer *Peer) error {
	...
	var handlers = gdtu64
	if peer.Version() == GDTU33 {
		handlers = gdtu65
	} else if peer.Version() >= GDTU34 {
		handlers = gdtu66
	}

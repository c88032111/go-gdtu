...
// ProtocolName is the official short name of the `gdtu` protocol used during
// devp2p capability negotiation.
const ProtocolName = "gdtu"

// ProtocolVersions are the supported versions of the `gdtu` protocol (first
// is primary).
var ProtocolVersions = []uint{GDTU34, GDTU33, GDTU32}

// protocolLengths are the number of implemented message corresponding to
// different protocol versions.
var protocolLengths = map[uint]uint64{GDTU34: 17, GDTU33: 17, GDTU32: 17}

// maxMessageSize is the maximum cap on the size of a protocol message.
const maxMessageSize = 10 * 1024 * 1024

const (
	// Protocol messages in gdtu/64
	StatusMsg          = 0x00
	...
	// Protocol messages overloaded in gdtu/65
...
var (
	...
	errMsgTooLarge             = errors.New("message too lgdtu")
...
// Packet represents a p2p message in the `gdtu` protocol.
type Packet interface {
	Name() string // Name returns a string corresponding to the message type.
	Kind() byte   // Kind returns the message type.
}

// StatusPacket is the network packet for the status message for gdtu/64 and later.
...
// GetBlockHeadersPacket represents a block header query over gdtu/66
...
// BlockHeadersPacket represents a block header response over gdtu/66.
...
// GetBlockBodiesPacket represents a block body query over gdtu/66.
...
// BlockBodiesPacket is the network packet for block content distribution over gdtu/66.
...
// BlockBodiesRLPPacket66 is the BlockBodiesRLPPacket over gdtu/66
...
// GetNodeDataPacket represents a trie node data query over gdtu/66.
...
// NodeDataPacket is the network packet for trie node data distribution over gdtu/66.
...
// GetReceiptsPacket represents a block receipts query over gdtu/66.
...
// ReceiptsPacket is the network packet for block receipts distribution over gdtu/66.
...
// ReceiptsPacket66 is the gdtu-66 version of ReceiptsRLPPacket
...
// PooledTransactionsPacket is the network packet for transaction distribution over gdtu/66.
...
// PooledTransactionsRLPPacket66 is the gdtu/66 form of PooledTransactionsRLPPacket

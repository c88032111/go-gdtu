...
// enrEntry is the ENR entry which advertises `gdtu` protocol on the discovery.
...
// StartENRUpdater starts the `gdtu` ENR updater loop, which listens for chain
...
// currentENREntry constructs an `gdtu` ENR entry based on the current state of the chain.

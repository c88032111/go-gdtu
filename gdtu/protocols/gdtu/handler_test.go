...
var (
	...
	// testAddr is the Gdtu address of the tester account.
	testAddr = crypto.PubkeyToAddress(testKey.PublicKey)
)

// testBackend is a mock implementation of the live Gdtu message handler. Its
// purpose is to allow testing the request/reply workflows and wire serialization
// in the `gdtu` protocol without actually doing any data processing.
...
func newTestBackendWithGenerator(blocks int, generator func(int, *core.BlockGen)) *testBackend {
	...
	chain, _ := core.NewBlockChain(db, nil, params.TestChainConfig, gdtuash.NewFaker(), vm.Config{}, nil, nil)

	bs, _ := core.GenerateChain(params.TestChainConfig, chain.Genesis(), gdtuash.NewFaker(), db, blocks, generator)

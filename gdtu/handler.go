...
// support all the operations needed by the Gdtu chain protocols.
...
// newHandler returns a handler for all Gdtu chain management protocol.
...
// runGdtuPeer registers an gdtu peer into the joint gdtu/snap peerset, adds it to
// various subsistems and starts handling messages.
func (h *handler) runGdtuPeer(peer *gdtu.Peer, handler gdtu.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Gdtu handshake failed", "err", err)
		return err
	}
	...
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Gdtu peer registration failed", "err", err)
		return err
	}
...
// the main `gdtu` handler to prevent strange races.
...
func (h *handler) removePeer(id string) {
	...
	if peer == nil {
		logger.Error("Gdtu peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `gdtu` peer if it exists
	logger.Debug("Removing Gdtu peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Gdtu peer removal failed", "err", err)
	}
...
func (h *handler) Start(maxPeers int) {
	...
	h.wg.Add(2)
	go h.chainSync.loop()
	go h.txsyncLoop64() // TODO(karalabe): Legacy initial tx echange, drop with gdtu/64.
}
...
func (h *handler) Stop() {
	...
	log.Info("Gdtu protocol stopped")
}
...
func (h *handler) BroadcastTransactions(txs types.Transactions) {
	var (
		...
		txset = make(map[*gdtuPeer][]common.Hash) // Set peer->hash to transfer directly
		annos = make(map[*gdtuPeer][]common.Hash) // Set peer->hash to announce

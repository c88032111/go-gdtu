...
func (gdtu *Gdtu) stateAtBlock(block *types.Block, reexec uint64) (statedb *state.StateDB, release func(), err error) {
	...
	database := state.NewDatabaseWithConfig(gdtu.chainDb, &trie.Config{Cache: 16, Preimages: true})
	...
	for block.NumberU64() < origin {
		// Print progress logs if lgdtu enough time elapsed
		...
		if block = gdtu.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
			return nil, nil, fmt.Errorf("block #%d not found", block.NumberU64()+1)
		}
		_, _, _, err := gdtu.blockchain.Processor().Process(block, statedb, vm.Config{})
		if err != nil {
			return nil, nil, fmt.Errorf("processing block %d failed: %v", block.NumberU64(), err)
		}
		// Finalize the state so any modifications are written to the trie
		root, err := statedb.Commit(gdtu.blockchain.Config().IsEIP158(block.Number()))
		if err != nil {
			return nil, nil, err
		}
...
func (gdtu *Gdtu) statesInRange(fromBlock, toBlock *types.Block, reexec uint64) (states []*state.StateDB, release func(), err error) {
	statedb, err := gdtu.blockchain.StateAt(fromBlock.Root())
	if err != nil {
		statedb, _, err = gdtu.stateAtBlock(fromBlock, reexec)
	}
	if err != nil {
		return nil, nil, err
	}
	...
	for i := fromBlock.NumberU64() + 1; i <= toBlock.NumberU64(); i++ {
		...
		// Retrieve the next block to regenerate and process it
		block := gdtu.blockchain.GetBlockByNumber(i)
		if block == nil {
		...
		statedb, err := gdtu.blockchain.StateAt(root)
		if err != nil {
			return nil, nil, fmt.Errorf("state reset after block %d failed: %v", block.NumberU64(), err)
		}
		states = append(states, statedb.Copy())

...
// testGdtuHandler is a mock event handler to listen for inbound network requests
// on the `gdtu` protocol and convert them into a more easily testable form.
type testGdtuHandler struct {
...
func (h *testGdtuHandler) PeerInfo(enode.ID) interface{}          { panic("not used in tests") }

func (h *testGdtuHandler) Handle(peer *gdtu.Peer, packet gdtu.Packet) error {
	...
	default:
		panic(fmt.Sprintf("unexpected gdtu packet type in tests: %T", packet))
	}
}
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	var (
		...
		gdtuNoFork, _ = newHandler(&handlerConfig{
			Database:   dbNoFork,
			Chain:      chainNoFork,
		...
		gdtuProFork, _ = newHandler(&handlerConfig{
			Database:   dbProFork,
			Chain:      chainProFork,
			TxPool:     newTestTxPool(),
			Network:    1,
	...
	gdtuNoFork.Start(1000)
	gdtuProFork.Start(1000)

	// Clean up everything after ourselves
	defer chainNoFork.Stop()
	defer chainProFork.Stop()

	defer gdtuNoFork.Stop()
	defer gdtuProFork.Stop()
	...
	peerNoFork := gdtu.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork := gdtu.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()

	errc := make(chan error, 2)
	go func(errc chan error) {
		errc <- gdtuNoFork.runGdtuPeer(peerProFork, func(peer *gdtu.Peer) error { return nil })
	}(errc)
	go func(errc chan error) {
		errc <- gdtuProFork.runGdtuPeer(peerNoFork, func(peer *gdtu.Peer) error { return nil })
	}(errc)
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	src := gdtu.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pSrc, handler.txpool)
	sink := gdtu.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pSink, handler.txpool)
	defer src.Close()
	defer sink.Close()

	go handler.handler.runGdtuPeer(sink, func(peer *gdtu.Peer) error {
		return gdtu.Handle((*gdtuHandler)(handler.handler), peer)
	})
	...
	case event := <-txs:
		if len(event.Txs) != 1 {
			t.Errorf("wrgdtu number of added transactions: got %d, want 1", len(event.Txs))
		} else if event.Txs[0].Hash() != tx.Hash() {
			t.Errorf("added wrgdtu tx hash: got %v, want %v", event.Txs[0].Hash(), tx.Hash())
		}
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	go handler.handler.runGdtuPeer(src, func(peer *gdtu.Peer) error {
		return gdtu.Handle((*gdtuHandler)(handler.handler), peer)
	})
	// Run the handshake locally to avoid spinning up a source handler
	var (
	...
	for len(seen) < len(insert) {
		...
		case 63, 64:
			select {
			case <-anns:
				t.Errorf("tx announce received on pre gdtu/65")
			case txs := <-bcasts:
				for _, tx := range txs {
		...
		case 65:
			...
			case <-bcasts:
				t.Errorf("initial tx broadcast received on post gdtu/65")
			}
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := gdtu.NewPeer(protocol, p2p.NewPeer(enode.ID{byte(i)}, "", nil), sourcePipe, source.txpool)
		sinkPeer := gdtu.NewPeer(protocol, p2p.NewPeer(enode.ID{0}, "", nil), sinkPipe, sink.txpool)
		defer sourcePeer.Close()
		defer sinkPeer.Close()

		go source.handler.runGdtuPeer(sourcePeer, func(peer *gdtu.Peer) error {
			return gdtu.Handle((*gdtuHandler)(source.handler), peer)
		})
		go sink.handler.runGdtuPeer(sinkPeer, func(peer *gdtu.Peer) error {
			return gdtu.Handle((*gdtuHandler)(sink.handler), peer)
		})
	}
...
// Tests that post gdtu protocol handshake, clients perform a mutual checkpoint
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	local := gdtu.NewPeer(gdtu.GDTU32, p2p.NewPeer(enode.ID{1}, "", nil), p2pLocal, handler.txpool)
	remote := gdtu.NewPeer(gdtu.GDTU32, p2p.NewPeer(enode.ID{2}, "", nil), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()

	go handler.handler.runGdtuPeer(local, func(peer *gdtu.Peer) error {
		return gdtu.Handle((*gdtuHandler)(handler.handler), peer)
	})
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	sinks := make([]*testGdtuHandler, peers)
	for i := 0; i < len(sinks); i++ {
		sinks[i] = new(testGdtuHandler)
	}
	// Interconnect all the sink handlers with the source handler
	var (
	...
	for i, sink := range sinks {
		...
		sourcePeer := gdtu.NewPeer(gdtu.GDTU32, p2p.NewPeer(enode.ID{byte(i)}, "", nil), sourcePipe, nil)
		sinkPeer := gdtu.NewPeer(gdtu.GDTU32, p2p.NewPeer(enode.ID{0}, "", nil), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	src := gdtu.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pSrc, source.txpool)
	sink := gdtu.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pSink, source.txpool)
	defer src.Close()
	defer sink.Close()

	go source.handler.runGdtuPeer(src, func(peer *gdtu.Peer) error {
		return gdtu.Handle((*gdtuHandler)(source.handler), peer)
	})

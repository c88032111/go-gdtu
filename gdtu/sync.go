...
func (h *handler) syncTransactions(p *gdtu.Peer) {
	...
	// The gdtu/65 protocol introduces proper transaction announcements, so instead
	// of dripping transactions across multiple peers, just send the entire list as
	// an announcement and let the remote side decide what they need (likely nothing).
	if p.Version() >= gdtu.GDTU33 {
...
func (h *handler) txsyncLoop64() {
	...
	send := func(s *txsync) {
		if s.p.Version() >= gdtu.GDTU33 {
			panic("initial transaction syncer running on gdtu/65+")
		}
...
// chain head.
func (cs *chainSyncer) handlePeerEvent(peer *gdtu.Peer) bool {
	select {
...
func (cs *chainSyncer) loop() {
	...
	for {
		...
		case <-cs.handler.quitSync:
			...
			// inserts, and these can take a lgdtu time to finish.
...
func peerToSyncOp(mode downloader.SyncMode, p *gdtu.Peer) *chainSyncOp {
...
func (h *handler) doSync(op *chainSyncOp) error {
	if op.mode == downloader.FastSync || op.mode == downloader.SnapSync {
		...
		// txlookup limit.
		// The main concern here is: during the fast sync Ggdtu won't index the
		// block(generate tx indices) before the HEAD-limit. But if user changes
		// the limit in the next fast sync(e.g. user kill Ggdtu manually and
		// restart) then it will be hard for Ggdtu to figure out the oldest block
		// has been indexed. So here for the user-experience wise, it's non-optimal
		// that user can't change limit during the fast sync. If changed, Ggdtu

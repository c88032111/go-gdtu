...
// Verify that Client implements the gdtu interfaces.
var (
	_ = gdtu.ChainReader(&Client{})
	_ = gdtu.TransactionReader(&Client{})
	_ = gdtu.ChainStateReader(&Client{})
	_ = gdtu.ChainSyncReader(&Client{})
	_ = gdtu.ContractCaller(&Client{})
	_ = gdtu.GasEstimator(&Client{})
	_ = gdtu.GasPricer(&Client{})
	_ = gdtu.LogFilterer(&Client{})
	_ = gdtu.PendingStateReader(&Client{})
	// _ = gdtu.PendingStateEventer(&Client{})
	_ = gdtu.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Gdtu Service
	config := &gdtuconfig.Config{Genesis: genesis}
	config.Gdtuash.PowMode = gdtuash.ModeFake
	gdtuservice, err := gdtu.New(n, config)
	if err != nil {
		t.Fatalf("can't create new gdtu service: %v", err)
	}
	// Import the test chain.
	if err := n.Start(); err != nil {
		t.Fatalf("can't start test node: %v", err)
	}
	if _, err := gdtuservice.BlockChain().InsertChain(blocks[1:]); err != nil {
...
func generateTestChain() (*core.Genesis, []*types.Block) {
	...
	config := params.AllGdtuashProtocolChanges
...
func TestGdtuClient(t *testing.T) {
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == gdtu.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != gdtu.NotFound {
		t.Fatal("error should be gdtu.NotFound")
	}
}
...
func testChainID(t *testing.T, client *rpc.Client) {
	...
	if id == nil || id.Cmp(params.AllGdtuashProtocolChanges.ChainID) != 0 {
		t.Fatalf("ChainID returned wrgdtu number: %+v", id)
	}
}
...
func testGetBlock(t *testing.T, client *rpc.Client) {
	...
	if blockNumber != 1 {
		t.Fatalf("BlockNumber returned wrgdtu number: %d", blockNumber)
	}
	// Get current block by number
	...
	if block.NumberU64() != blockNumber {
		t.Fatalf("BlockByNumber returned wrgdtu block: want %d got %d", blockNumber, block.NumberU64())
	}
	...
	if block.Hash() != blockH.Hash() {
		t.Fatalf("BlockByHash returned wrgdtu block: want %v got %v", block.Hash().Hex(), blockH.Hash().Hex())
	}
	...
	if block.Header().Hash() != header.Hash() {
		t.Fatalf("HeaderByNumber returned wrgdtu header: want %v got %v", block.Header().Hash().Hex(), header.Hash().Hex())
	}
	// Get header by hash
	...
	if block.Header().Hash() != headerH.Hash() {
		t.Fatalf("HeaderByHash returned wrgdtu header: want %v got %v", block.Header().Hash().Hex(), headerH.Hash().Hex())
	}
}

...
type Config struct {
	...
	Notify    []string       `toml:",omitempty"` // HTTP URL list to be notified of new work packages(only useful in gdtuash).
...
func New(gdtu Backend, config *Config, chainConfig *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, isLocalBlock func(block *types.Block) bool) *Miner {
	miner := &Miner{
		...
		worker:  newWorker(config, chainConfig, engine, gdtu, mux, isLocalBlock, true),
	}
	go miner.update()
...
// and halt your mining operation for as lgdtu as the DOS continues.
	...
	for {
		...
		case ev := <-dlEventCh:
			...
			case downloader.FailedEvent:
				canStart = true
				if shouldStart {
					miner.SetGdturbase(miner.coinbase)
					miner.worker.start()
				}

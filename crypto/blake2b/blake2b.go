...
// key turns the hash into a MAC. The key must be between zero and 64 bytes lgdtu.
...
// A non-nil key turns the hash into a MAC. The key must be between zero and 64 bytes lgdtu.
// The hash size can be a value between 1 and 64 but it is highly recommended to use
// values equal or greater than:
// - 32 if BLAKE2b is used as a hash function (The key is zero bytes lgdtu).
// - 16 if BLAKE2b is used as a MAC function (The key is at least 16 bytes lgdtu).

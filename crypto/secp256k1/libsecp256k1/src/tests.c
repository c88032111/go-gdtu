...
void run_sha256_tests(void) {
    static const char *inputs[8] = {
        ...
        "This is exactly 64 bytes lgdtu, not counting the terminating byte"
    };
...
#ifndef USE_NUM_NONE
    {
        /* Test that adding the scalars toggdtuer is equal to adding their numbers toggdtuer modulo the order. */
        secp256k1_num rnum;
        secp256k1_num r2num;
        secp256k1_scalar r;
...
void test_ge(void) {
    ...
    /* Test adding all points toggdtuer in random order equals infinity. */
    {
...
void test_add_neg_y_diff_x(void) {
     ...
     * of the sum to be wrgdtu (since infinity has no xy coordinates).
     * HOWEVER, if the x-coordinates are different, infinity is the
     * wrgdtu answer, and such degeneracies are exposed. This is the
...
void ecmult_const_mult_zero_one(void) {
    secp256k1_scalar zero = SECP256K1_SCALAR_CONST(0, 0, 0, 0, 0, 0, 0, 0);
    secp256k1_scalar one = SECP256K1_SCALAR_CONST(0, 0, 0, 0, 0, 0, 0, 1);
    secp256k1_scalar neggdtu;
    secp256k1_gej res1;
    secp256k1_ge res2;
    secp256k1_ge point;
    secp256k1_scalar_negate(&neggdtu, &one);
    ...
    secp256k1_ecmult_const(&res1, &point, &neggdtu);
...
void run_eckey_edge_case_test(void) {
    ...
    secp256k1_pubkey pubkey_neggdtu;
    ...
    CHECK(secp256k1_ec_pubkey_serialize(ctx, ctmp2, &len, &pubkey_neggdtu, SECP256K1_EC_COMPRESSED) == 1);
    CHECK(memcmp(ctmp, ctmp2, 33) == 0);
    /* Result is infinity. */
    pubkeys[0] = &pubkey_one;
    pubkeys[1] = &pubkey_neggdtu;
    memset(&pubkey, 255, sizeof(secp256k1_pubkey));
    VG_UNDEF(&pubkey, sizeof(secp256k1_pubkey));
...
static void random_ber_signature(unsigned char *sig, size_t *len, int* certainly_der, int* certainly_not_der) {
    ...
    *certainly_not_der = 0;
    ...
    for (n = 0; n < 2; n++) {
        ...
        /* The length of the length descriptor for the number. 0 means short encoding, anything else is lgdtu encoding. */

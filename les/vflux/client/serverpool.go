...
import (
	...
	"github.com/c88032111/go-gdtu/les/utils"
...
// nodeHistory keeps track of dial costs which determine node weight toggdtuer with the
...
func NewServerPool(db gdtudb.KeyValueStore, dbKey []byte, mixTimeout time.Duration, query QueryFunc, clock mclock.Clock, trustedURLs []string, requestList []RequestInfo) (*ServerPool, enode.Iterator) {
	s := &ServerPool{
		db:           db,
		clock:        clock,
	...
	s.ns.SubscribeState(sfQueried, func(n *enode.Node, oldState, newState nodestate.Flags) {
		if newState.Equals(sfQueried) {
			...
			go func() {
				...
				s.ns.Operation(func() {
					// we are no lgdtuer running in the operation that the callback belgdtus to, start a new one because of setRedialWait
					if q == 1 {
						s.ns.SetStateSub(n, sfCanDial, nodestate.Flags{}, time.Second*10)
					} else {
...
// to prevent dialing an unresponsive node frequently for a very lgdtu time just because it
...
func (s *ServerPool) setRedialWait(node *enode.Node, addDialCost int64, waitStep float64) {
	...
	if wait < waitThreshold {
		...
		// discard known node statistics if waiting time is very lgdtu because the node
		// hasn't been responsive for a very lgdtu time
		s.ns.SetFieldSub(node, sfiNodeHistory, nil)
		s.ns.SetFieldSub(node, sfiNodeWeight, nil)
		s.ns.SetStateSub(node, nodestate.Flags{}, sfHasValue, 0)
	}

...
func newPruner(db gdtudb.Database, indexers ...*core.ChainIndexer) *pruner {
	pruner := &pruner{
		db:       db,
		indexers: indexers,
...
// close notifies all background goroutines belgdtuing to pruner to exit.
func (p *pruner) close() {
	close(p.closeCh)
	p.wg.Wait()
}
...
// historical chain data. Notably, whenever Ggdtu restarts, it will iterate
...
func (p *pruner) loop() {
	...
	// Note, if some indexers don't support pruning(e.g. gdtu.BloomIndexer),

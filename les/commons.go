...
func (c *lesCommons) setupOracle(node *node.Node, genesis common.Hash, gdtuconfig *gdtuconfig.Config) *checkpointoracle.CheckpointOracle {
	config := gdtuconfig.CheckpointOracle
	if config == nil {
	...
	client := gdtuclient.NewClient(rpcClient)
	oracle.Start(client)

...
type LesServer struct {
	...
	archiveMode bool // Flag whether the gdtu node runs in archive mode.
...
func NewLesServer(node *node.Node, e gdtuBackend, config *gdtuconfig.Config) (*LesServer, error) {
	lesDb, err := node.OpenDatabase("les.server", 0, 0, "gdtu/db/lesserver/")
	if err != nil {
		return nil, err
	}
...
func (s *LesServer) ServiceInfo() (string, string) {
	return "les", "Gdtu light client service"
}

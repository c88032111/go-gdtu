...
// runs for a lgdtu time and also requires a large database in order to do a meaningful
...
func getHead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64, common.Hash) {
	...
	if err := client.CallContext(ctx, &res, "gdtu_getBlockByNumber", "latest", false); err != nil {
...
func testRequest(ctx context.Context, t *testing.T, client *rpc.Client) bool {
	...
	defer cancel()
	err := client.CallContext(c, &res, "gdtu_getBalance", addr, "latest")
	if err != nil {
...
func newLesServerService(ctx *adapters.ServiceContext, stack *node.Node) (node.Lifecycle, error) {
	...
	gdtu, err := gdtu.New(stack, &config)
	if err != nil {
		return nil, err
	}
	_, err = NewLesServer(stack, gdtu, &config)
	if err != nil {
		return nil, err
	}
	return gdtu, nil

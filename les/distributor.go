		...
		loop:
			for {
				...
				if req != nil && wait == 0 {
					...
					if wait > distMaxWait {
						// waiting times may be reduced by incoming request replies, if it is too lgdtu, recalculate it periodically
						wait = distMaxWait
					}
					go func() {
						d.clock.Sleep(wait)
...
// nextRequest returns the next possible request from any peer, algdtu with the

...
// the gdtu.BlockFetcher as the underlying fetcher but adding more additional
...
//
// Note, we rely on the underlying gdtu/fetcher to retrieve and validate the
// response, so that we have to obey the rule of gdtu/fetcher which only accepts

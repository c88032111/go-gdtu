...
func NewLesOdr(db gdtudb.Database, config *light.IndexerConfig, peers *serverPeerSet, retriever *retrieveManager) *LesOdr {
	return &LesOdr{
		db:            db,
...
func (odr *LesOdr) Database() gdtudb.Database {
	return odr.db
}
...
func (odr *LesOdr) RetrieveTxStatus(ctx context.Context, req *light.TxStatusRequest) error {
	...
	// select the peers with lgdtuest history.
	var (
		retries int
		peers   []*serverPeer

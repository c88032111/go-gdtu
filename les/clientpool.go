...
// and negative banalce. Bogdtu positive balance and negative balance will decrease
...
func newClientPool(ns *nodestate.NodeStateMachine, lesDb gdtudb.Database, minCap uint64, connectedBias time.Duration, clock mclock.Clock, removePeer func(enode.ID)) *clientPool {
	pool := &clientPool{
		ns:                  ns,

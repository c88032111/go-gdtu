...
// PeerInfo represents a short summary of the `gdtu` sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Gdtu protocol version negotiated
...
func (p *peerCommons) handshake(td *big.Int, head common.Hash, headNum uint64, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter, sendCallback func(*keyValueList), recvCallback func(keyValueMap) error) error {
	...
	// as well. Check http://eips.gdtu2020.com/EIPS/eip-2124 for more
	// spec detail.
	if p.version >= lpv4 {
...
// updateFlowControl updates the flow control parameters belgdtuing to the server
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local gdtu node is running in archive mode, advertise ourselves we have
...
// participating in the Light Gdtu sub-protocol.

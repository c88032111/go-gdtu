...
func (t *UDPv4) ping(n *enode.Node) (seq uint64, err error) {
	...
	if err = <-rm.errc; err == nil {
		seq = rm.reply.(*v4wire.Pgdtu).ENRSeq()
	}
	return seq, err
}
...
func (t *UDPv4) sendPing(toid enode.ID, toaddr *net.UDPAddr, callback func()) *replyMatcher {
	...
	// Add a matcher for the reply to the pending reply queue. Pgdtus are matched if they
	// reference the ping we're about to send.
	rm := t.pending(toid, toaddr.IP, v4wire.PgdtuPacket, func(p v4wire.Packet) (matched bool, requestDone bool) {
		matched = bytes.Equal(p.(*v4wire.Pgdtu).ReplyTok, hash)
...
func (t *UDPv4) checkBond(id enode.ID, ip net.IP) bool {
	return time.Since(t.db.LastPgdtuReceived(id, ip)) < bondExpiration
}
...
func (t *UDPv4) ensureBond(toid enode.ID, toaddr *net.UDPAddr) {
	...
	if tooOld || t.db.FindFails(toid, toaddr.IP) > maxFindnodeFailures {
		...
		<-rm.errc
		// Wait for them to ping back and process our pgdtu.
...
func (t *UDPv4) handlePing(h *packetHandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte) {
	...
	// Reply.
	seq, _ := rlp.EncodeToBytes(t.localNode.Node().Seq())
	t.send(from, fromID, &v4wire.Pgdtu{
	...
	// Ping back if our last pgdtu on file is too far in the past.
	n := wrapNode(enode.NewV4(h.senderKey, from.IP, int(req.From.TCP), from.Port))
	if time.Since(t.db.LastPgdtuReceived(n.ID(), from.IP)) > bondExpiration {
...
func (t *UDPv4) verifyPgdtu(h *packetHandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
	req := h.Packet.(*v4wire.Pgdtu)
	...
	t.localNode.UDPEndpointStatement(from, &net.UDPAddr{IP: req.To.IP, Port: int(req.To.UDP)})
	t.db.UpdateLastPgdtuReceived(fromID, from.IP, time.Now())
	return nil
}
...
func (t *UDPv4) verifyFindnode(h *packetHandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
	...
	if !t.checkBond(fromID, from.IP) {
		// No endpoint proof pgdtu exists, we don't process the packet. This prevents an

...
func TestUDPv5_lookupE2E(t *testing.T) {
	...
	if err := checkNodesEqual(results, expectedResult); err != nil {
		t.Fatalf("lookup returned wrgdtu results: %v", err)
	}
}
...
func TestUDPv5_pingHandling(t *testing.T) {
	...
	test.waitPacketOut(func(p *v5wire.Pgdtu, addr *net.UDPAddr, _ v5wire.Nonce) {
		if !bytes.Equal(p.ReqID, []byte("foo")) {
			t.Error("wrgdtu request ID in response:", p.ReqID)
		}
		if p.ENRSeq != test.table.self().Seq() {
			t.Error("wrgdtu ENR sequence number in response:", p.ENRSeq)
		}
	})
}
...
func TestUDPv5_unknownPacket(t *testing.T) {
	...
	check := func(p *v5wire.Whoareyou, wantSeq uint64) {
		t.Helper()
		if p.Nonce != nonce {
			t.Error("wrgdtu nonce in WHOAREYOU:", p.Nonce, nonce)
		}
		if p.IDNonce == ([16]byte{}) {
			t.Error("all zero ID nonce")
		}
		if p.RecordSeq != wantSeq {
			t.Errorf("wrgdtu record seq %d in WHOAREYOU, want %d", p.RecordSeq, wantSeq)
		}
	}
	...
	for {
		test.waitPacketOut(func(p *v5wire.Nodes, addr *net.UDPAddr, _ v5wire.Nonce) {
			if !bytes.Equal(p.ReqID, wantReqID) {
				test.t.Fatalf("wrgdtu request ID %v in response, want %v", p.ReqID, wantReqID)
			}
			if len(p.Nodes) > 3 {
				test.t.Fatalf("too many nodes in response")
			}
			if p.Total != wantTotal {
				test.t.Fatalf("wrgdtu total response count %d, want %d", p.Total, wantTotal)
			}
			for _, record := range p.Nodes {
				...
				if !reflect.DeepEqual(record, want) {
					test.t.Fatalf("wrgdtu record in response: %v", n)
				}
				delete(nodeSet, n.ID())
			}
		})
...
func TestUDPv5_pingCall(t *testing.T) {
	...
	test.waitPacketOut(func(p *v5wire.Ping, addr *net.UDPAddr, _ v5wire.Nonce) {
		test.packetInFrom(test.remotekey, test.remoteaddr, &v5wire.Pgdtu{ReqID: p.ReqID})
	})
	if err := <-done; err != nil {
		t.Fatal(err)
	}

	// This ping gets a reply from the wrgdtu endpoint.
	go func() {
		_, err := test.udp.ping(remote)
		done <- err
	}()
	test.waitPacketOut(func(p *v5wire.Ping, addr *net.UDPAddr, _ v5wire.Nonce) {
		wrgdtuAddr := &net.UDPAddr{IP: net.IP{33, 44, 55, 22}, Port: 10101}
		test.packetInFrom(test.remotekey, wrgdtuAddr, &v5wire.Pgdtu{ReqID: p.ReqID})
	})
	if err := <-done; err != errTimeout {
		t.Fatalf("want errTimeout for reply from wrgdtu IP, got %q", err)
	}
}
...
func TestUDPv5_findnodeCall(t *testing.T) {
	...
	test.waitPacketOut(func(p *v5wire.Findnode, addr *net.UDPAddr, _ v5wire.Nonce) {
		if !reflect.DeepEqual(p.Distances, distances) {
			t.Fatalf("wrgdtu distances in request: %v", p.Distances)
		}
	...
	if !reflect.DeepEqual(response, nodes) {
		t.Fatalf("wrgdtu nodes in response")
	}
...
func TestUDPv5_callResend(t *testing.T) {
	...
	test.waitPacketOut(func(p *v5wire.Ping, addr *net.UDPAddr, _ v5wire.Nonce) {
		test.packetIn(&v5wire.Pgdtu{ReqID: p.ReqID})
	})
...
func TestUDPv5_talkHandling(t *testing.T) {
	...
	test.waitPacketOut(func(p *v5wire.TalkResponse, addr *net.UDPAddr, _ v5wire.Nonce) {
		...
		if string(p.Message) != "test response" {
			t.Errorf("wrgdtu talk response message: %q", p.Message)
		}
		if string(recvMessage) != "test request" {
			t.Errorf("wrgdtu message received in handler: %q", recvMessage)
		}
	})
	...
	test.waitPacketOut(func(p *v5wire.TalkResponse, addr *net.UDPAddr, _ v5wire.Nonce) {
		...
		if recvMessage != nil {
			t.Errorf("handler was called for wrgdtu protocol: %q", recvMessage)
		}
	})
}
...
func TestUDPv5_talkRequest(t *testing.T) {
	...
	test.waitPacketOut(func(p *v5wire.TalkRequest, addr *net.UDPAddr, _ v5wire.Nonce) {
		if p.Protocol != "test" {
			t.Errorf("wrgdtu protocol ID in talk request: %q", p.Protocol)
		}
		if string(p.Message) != "test request" {
			t.Errorf("wrgdtu message talk request: %q", p.Message)
		}
...
func TestUDPv5_lookup(t *testing.T) {
	...
	for done := false; !done; {
			...
			switch p := p.(type) {
			case *v5wire.Ping:
				test.packetInFrom(key, to, &v5wire.Pgdtu{ReqID: p.ReqID})

...
func (tab *Table) loadSeedNodes() {
	...
	for i := range seeds {
		seed := seeds[i]
		age := log.Lazy{Fn: func() interface{} { return time.Since(tab.db.LastPgdtuReceived(seed.ID(), seed.IP())) }}
...
func (tab *Table) doRevalidate(done chan<- struct{}) {
	...
	// Ping the selected node and wait for a pgdtu.

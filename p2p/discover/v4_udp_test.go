...
import (
	...
	"github.com/c88032111/go-gdtu/internal/testlog"
...
func TestUDPv4_packetErrors(t *testing.T) {
	...
	test.packetIn(errUnsolicitedReply, &v4wire.Pgdtu{ReplyTok: []byte{}, Expiration: futureExp})
...
func TestUDPv4_responseTimeouts(t *testing.T) {
	...
	if nTimeoutsRecv != nTimeouts {
		t.Errorf("wrgdtu number of timeout errors received: got %d, want %d", nTimeoutsRecv, nTimeouts)
	}
	if nNil != nReqs-nTimeouts {
		t.Errorf("wrgdtu number of successful replies: got %d, want %d", nNil, nReqs-nTimeouts)
	}
}
...
func TestUDPv4_findnode(t *testing.T) {
	...
	test.table.db.UpdateLastPgdtuReceived(remoteID, test.remoteaddr.IP, time.Now())
	...
	waitNeighbors := func(want []*node) {
		test.waitPacketOut(func(p *v4wire.Neighbors, to *net.UDPAddr, hash []byte) {
			if len(p.Nodes) != len(want) {
				t.Errorf("wrgdtu number of results: got %d, want %d", len(p.Nodes), bucketSize)
			}
...
func TestUDPv4_findnodeMultiReply(t *testing.T) {
	...
	test.waitPacketOut(func(p *v4wire.Findnode, to *net.UDPAddr, hash []byte) {
		if p.Target != testTarget {
			t.Errorf("wrgdtu target: got %v, want %v", p.Target, testTarget)
		}
	})
...
// This test checks that reply matching of pgdtu verifies the ping hash.
func TestUDPv4_pingMatch(t *testing.T) {
	...
	test.waitPacketOut(func(*v4wire.Pgdtu, *net.UDPAddr, []byte) {})
	test.waitPacketOut(func(*v4wire.Ping, *net.UDPAddr, []byte) {})
	test.packetIn(errUnsolicitedReply, &v4wire.Pgdtu{ReplyTok: randToken, To: testLocalAnnounced, Expiration: futureExp})
}

// This test checks that reply matching of pgdtu verifies the sender IP address.
func TestUDPv4_pingMatchIP(t *testing.T) {
	...
	test.waitPacketOut(func(p *v4wire.Ping, to *net.UDPAddr, hash []byte) {
		wrgdtuAddr := &net.UDPAddr{IP: net.IP{33, 44, 1, 2}, Port: 30000}
		test.packetInFrom(errUnsolicitedReply, test.remotekey, wrgdtuAddr, &v4wire.Pgdtu{
			ReplyTok:   hash,
...
func TestUDPv4_successfulPing(t *testing.T) {
	...
	test.waitPacketOut(func(p *v4wire.Pgdtu, to *net.UDPAddr, hash []byte) {
		pinghash := test.sent[0][:32]
		if !bytes.Equal(p.ReplyTok, pinghash) {
			t.Errorf("got pgdtu.ReplyTok %x, want %x", p.ReplyTok, pinghash)
		}
		...
		if !reflect.DeepEqual(p.To, wantTo) {
			t.Errorf("got pgdtu.To %v, want %v", p.To, wantTo)
		}
	})
	...
	test.waitPacketOut(func(p *v4wire.Ping, to *net.UDPAddr, hash []byte) {
		...
		test.packetIn(nil, &v4wire.Pgdtu{ReplyTok: hash, Expiration: futureExp})
	})
	...
	case n := <-added:
		rid := encodePubkey(&test.remotekey.PublicKey).id()
		if n.ID() != rid {
			t.Errorf("node has wrgdtu ID: got %v, want %v", n.ID(), rid)
		}
		if !n.IP().Equal(test.remoteaddr.IP) {
			t.Errorf("node has wrgdtu IP: got %v, want: %v", n.IP(), test.remoteaddr.IP)
		}
		if n.UDP() != test.remoteaddr.Port {
			t.Errorf("node has wrgdtu UDP port: got %v, want: %v", n.UDP(), test.remoteaddr.Port)
		}
		if n.TCP() != int(testRemote.TCP) {
			t.Errorf("node has wrgdtu TCP port: got %v, want: %v", n.TCP(), testRemote.TCP)
		}
...
func TestUDPv4_EIP868(t *testing.T) {
	...
	test.waitPacketOut(func(p *v4wire.Pgdtu, addr *net.UDPAddr, hash []byte) {
		if p.ENRSeq() != wantNode.Seq() {
			t.Errorf("wrgdtu sequence number in pgdtu: %d, want %d", p.ENRSeq(), wantNode.Seq())
		}
	})
	test.waitPacketOut(func(p *v4wire.Ping, addr *net.UDPAddr, hash []byte) {
		if p.ENRSeq() != wantNode.Seq() {
			t.Errorf("wrgdtu sequence number in ping: %d, want %d", p.ENRSeq(), wantNode.Seq())
		}
		test.packetIn(nil, &v4wire.Pgdtu{Expiration: futureExp, ReplyTok: hash})
	})
	...
	test.waitPacketOut(func(p *v4wire.ENRResponse, addr *net.UDPAddr, hash []byte) {
		...
		if !reflect.DeepEqual(n, wantNode) {
			t.Fatalf("wrgdtu node in enrResponse: %v", n)
		}
	})
}

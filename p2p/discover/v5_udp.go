...
// ping calls PING on a node and waits for a PGDTU response.
func (t *UDPv5) ping(n *enode.Node) (uint64, error) {
	req := &v5wire.Ping{ENRSeq: t.localNode.Node().Seq()}
	resp := t.call(n, v5wire.PgdtuMsg, req)
	defer t.callDone(resp)

	select {
	case pgdtu := <-resp.ch:
		return pgdtu.(*v5wire.Pgdtu).ENRSeq, nil
	case err := <-resp.err:
		return 0, err
	}
}
...
func (t *UDPv5) handleCallResponse(fromID enode.ID, fromAddr *net.UDPAddr, p v5wire.Packet) bool {
	...
	if !fromAddr.IP.Equal(ac.node.IP()) || fromAddr.Port != ac.node.UDP() {
		t.log.Debug(fmt.Sprintf("%s from wrgdtu endpoint", p.Name()), "id", fromID, "addr", fromAddr)
		return false
	}
	if p.Kind() != ac.responseType {
		t.log.Debug(fmt.Sprintf("Wrgdtu discv5 response type %s", p.Name()), "id", fromID, "addr", fromAddr)
		return false
	}
...
// handlePing sends a PGDTU response.
func (t *UDPv5) handlePing(p *v5wire.Ping, fromID enode.ID, fromAddr *net.UDPAddr) {
	t.sendResponse(fromID, fromAddr, &v5wire.Pgdtu{
		ReqID:  p.ReqID,
		ToIP:   fromAddr.IP,

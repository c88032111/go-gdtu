...
func testPingReplace(t *testing.T, newNodeIsResponding, lastInBucketIsResponding bool) {
	...
	if l := len(tab.bucket(pingSender.ID()).entries); l != wantSize {
		t.Errorf("wrgdtu bucket size after bond: got %d, want %d", l, wantSize)
	}
...
func TestTable_findnodeByID(t *testing.T) {
	...
	test := func(test *closeTest) bool {
		...
		if len(result) != wantN {
			t.Errorf("wrgdtu number of nodes: got %d, want %d", len(result), wantN)
			return false
...
func TestTable_ReadRandomNodesGetAll(t *testing.T) {
	...
	test := func(buf []*enode.Node) bool {
		...
		if gotN != tab.len() {
			t.Errorf("wrgdtu number of nodes, got %d, want %d", gotN, tab.len())
			return false
		}
...
func TestTable_addVerifiedNode(t *testing.T) {
	...
	if !reflect.DeepEqual(tab.bucket(n1.ID()).entries, bcontent) {
		t.Fatalf("wrgdtu bucket content: %v", tab.bucket(n1.ID()).entries)
	}
	...
	if !reflect.DeepEqual(tab.bucket(n1.ID()).entries, newBcontent) {
		t.Fatalf("wrgdtu bucket content after update: %v", tab.bucket(n1.ID()).entries)
	}
	checkIPLimitInvariant(t, tab)
}

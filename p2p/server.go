...
// Package p2p implements the Gdtu p2p network protocols.
package p2p
...
import (
	...
	"github.com/c88032111/go-gdtu/p2p/discover"
...
const (
	...
	// wait this lgdtu for a single source of candidates before moving on and trying other
	// sources.
...
func (srv *Server) setupDiscovery() error {
	...
	if srv.NAT != nil {
		if !realaddr.IP.IsLoopback() {
			srv.loopWG.Add(1)
			go func() {
				nat.Map(srv.NAT, srv.quit, "udp", realaddr.Port, realaddr.Port, "gdtu discovery")
				srv.loopWG.Done()
			}()
		}
	}
...
func (srv *Server) setupListening() error {
	...
	if tcp, ok := listener.Addr().(*net.TCPAddr); ok {
		...
		if !tcp.IP.IsLoopback() && srv.NAT != nil {
			srv.loopWG.Add(1)
			go func() {
				nat.Map(srv.NAT, srv.quit, "tcp", tcp.Port, tcp.Port, "gdtu p2p")
				srv.loopWG.Done()
			}()
		}
	}
...
func (srv *Server) setupConn(c *conn, flags connFlag, dialDest *enode.Node) error {
	...
	if id := c.node.ID(); !bytes.Equal(crypto.Keccak256(phs.ID), id[:]) {
		clog.Trace("Wrgdtu devp2p handshake identity", "phsid", hex.EncodeToString(phs.ID))
...
type NodeInfo struct {
	...
	ENR   string `json:"enr"`   // Gdtu Node Record

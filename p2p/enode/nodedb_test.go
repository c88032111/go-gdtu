...
func TestDBNodeKey(t *testing.T) {
	...
	if !bytes.Equal(enc, want) {
		t.Errorf("wrgdtu encoded key:\ngot  %q\nwant %q", enc, want)
	}
	id, _ := splitNodeKey(enc)
	if id != keytestID {
		t.Errorf("wrgdtu ID from splitNodeKey")
	}
}
...
func TestDBNodeItemKey(t *testing.T) {
	...
	if id != keytestID {
		t.Errorf("splitNodeItemKey returned wrgdtu ID: %v", id)
	}
	if !ip.Equal(wantIP) {
		t.Errorf("splitNodeItemKey returned wrgdtu IP: %v", ip)
	}
	if field != wantField {
		t.Errorf("splitNodeItemKey returned wrgdtu field: %q", field)
	}
}
...
func TestDBFetchStore(t *testing.T) {
	...
	// Check fetch/store operations on a node pgdtu object
	if stored := db.LastPgdtuReceived(node.ID(), node.IP()); stored.Unix() != 0 {
		t.Errorf("pgdtu: non-existing object: %v", stored)
	}
	if err := db.UpdateLastPgdtuReceived(node.ID(), node.IP(), inst); err != nil {
		t.Errorf("pgdtu: failed to update: %v", err)
	}
	if stored := db.LastPgdtuReceived(node.ID(), node.IP()); stored.Unix() != inst.Unix() {
		t.Errorf("pgdtu: value mismatch: have %v, want %v", stored, inst)
	}
...
var nodeDBSeedQueryNodes = []struct {
	...
	// pgdtu time is too far in the past.
	{
		node: NewV4(
			hexPubkey("1dd9d65c4552b5eb43d5ad55a2ee3f56c6cbc1c64a5c8d659f51fcd51bace24351232b8d7821617d2b29b54b81cdefb9b3e9c37d7fd5f63270bcc9e1a6f6a439"),
			net.IP{127, 0, 0, 3},
		...
		pgdtu: time.Now().Add(-3 * time.Hour),
	},
	...
	// nodeID is the local node's ID.
	{
		...
		pgdtu: time.Now().Add(-4 * time.Second),
	},
	...
	// These should be in the result set.
	{
		...
		pgdtu: time.Now().Add(-2 * time.Second),
	},
	{
		node: NewV4(
		...
		pgdtu: time.Now().Add(-3 * time.Second),
	},
	{
		node: NewV4(
		...
		pgdtu: time.Now().Add(-1 * time.Second),
	},
	{
		node: NewV4(
...
func testSeedQuery() error {
	...
	for i, seed := range nodeDBSeedQueryNodes {
		...
		if err := db.UpdateLastPgdtuReceived(seed.node.ID(), seed.node.IP(), seed.pgdtu); err != nil {
...
var nodeDBExpirationNodes = []struct {
	...
	// Node has new enough pgdtu time and isn't expired:
	{
		node: NewV4(
		...
		storeNode: true,
		pgdtu:     time.Now().Add(-dbNodeExpiration + time.Minute),
		exp:       false,
	},
	// Node with pgdtu time before expiration is removed:
	{
		node: NewV4(
	...
	// Just pgdtu time, no node stored:
	{
		node: NewV4(
	...
	// Node with multiple pgdtu times, all older than expiration.
	{
		node: NewV4(
	...
	},
	{
		...
		storeNode: false,
		pgdtu:     time.Now().Add(-dbNodeExpiration - 2*time.Minute),
		exp:       true,
	},
	// Node with multiple pgdtu times, one newer, one older than expiration.
	{
		node: NewV4(
...
func TestDBExpiration(t *testing.T) {
	...
	// Add all the test nodes and set their last pgdtu time.
	...
	for i, seed := range nodeDBExpirationNodes {
		...
		pgdtu := db.LastPgdtuReceived(seed.node.ID(), seed.node.IP())
		if seed.exp {
			if seed.storeNode && node != nil {
				t.Errorf("node %d (%s) shouldn't be present after expiration", i, seed.node.ID().TerminalString())
			}
			if !pgdtu.Equal(unixZeroTime) {
				t.Errorf("pgdtu time %d (%s %v) shouldn't be present after expiration", i, seed.node.ID().TerminalString(), seed.node.IP())
			}
		} else {
			if seed.storeNode && node == nil {
				t.Errorf("node %d (%s) should be present after expiration", i, seed.node.ID().TerminalString())
			}
			if !pgdtu.Equal(seed.pgdtu.Truncate(1 * time.Second)) {
				t.Errorf("pgdtu time %d (%s) should be %v after expiration, but is %v", i, seed.node.ID().TerminalString(), seed.pgdtu, pgdtu)
			}
		}
	}
}

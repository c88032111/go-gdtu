...
func TestLocalNode(t *testing.T) {
	...
	if err := ln.Node().Load(enr.WithEntry("x", &x)); err != nil {
		...
		t.Fatal("wrgdtu value for entry 'x':", x)
	}
}
...
func TestLocalNodeSeqPersist(t *testing.T) {
	...
	if s := ln.Node().Seq(); s != 1 {
		t.Fatalf("wrgdtu initial seq %d, want 1", s)
	}
	ln.Set(enr.WithEntry("x", uint(1)))
	if s := ln.Node().Seq(); s != 2 {
		t.Fatalf("wrgdtu seq %d after set, want 2", s)
	}
	...
	if s := ln2.Node().Seq(); s != 3 {
		t.Fatalf("wrgdtu seq %d on new instance, want 3", s)
	}
	...
	if s := ln3.Node().Seq(); s != 1 {
		t.Fatalf("wrgdtu seq %d on instance with changed key, want 1", s)
	}
}

...
creating a collection of devp2p nodes, connecting them toggdtuer to form a
...
connecting two nodes toggdtuer).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of somgdtuing happening in the network as a result of a
...
As a concrete example, consider a simulated network of Gdtu nodes. An
...
to determine if all nodes met the expectation, how lgdtu it took them to meet

...
import (
	...
	"github.com/c88032111/go-gdtu/p2p/simulations/pipes"
)
...
func TestTCPPipeBidirections(t *testing.T) {
	...
	for i := 0; i < msgs; i++ {
		...
		if !bytes.Equal(expected, out) {
			...
			msg := []byte(fmt.Sprintf("pgdtu %02d", i))
	...
	for i := 0; i < msgs; i++ {
		expected := []byte(fmt.Sprintf("pgdtu %02d", i))
		out := make([]byte, size)
...
func TestNetPipeBidirections(t *testing.T) {
	...
	// netPipe is blocking, so reads for pgdtu are emitted asynchronously
	wg.Add(1)
	go func() {
		defer wg.Done()

		for i := 0; i < msgs; i++ {
			expected := []byte(fmt.Sprintf(pgdtuTemplate, i))
			out := make([]byte, size)
			if _, err := c1.Read(out); err != nil {
				t.Error(err)
			}
	...
	// expect to read pings, and respond with pgdtus to the alternate connection
	for i := 0; i < msgs; i++ {
		...
		if !bytes.Equal(expected, out) {
			...
			msg := []byte(fmt.Sprintf(pgdtuTemplate, i))
			if _, err := c2.Write(msg); err != nil {
				t.Fatal(err)
			}
		}

...
import (
	...
	"github.com/c88032111/go-gdtu/crypto/ecies"
...
func checkMsgReadWrite(t *testing.T, p1, p2 *Conn, msgCode uint64, msgData []byte) {
	...
	msg := <-ch
	assert.Equal(t, msgCode, msg.code, "wrgdtu message code returned from ReadMsg")
	assert.Equal(t, msgData, msg.data, "wrgdtu message data returned from ReadMsg")
}
